...
import mill.internal.BuildFileDiscovery
...
object DiscoveredBuildFiles {
  ...
  def parseBuildFiles(
    ...
    val (isDummy, foundRootBuildFileName) = BuildFileDiscovery.findRootBuildFiles(projectRoot)
    ...
    DiscoveredBuildFiles(seenScripts.toMap, errors.toSeq.map(_.error), seenPkgStatements.toMap)
  }
