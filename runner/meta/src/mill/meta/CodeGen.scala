...
object CodeGen {
  ...
  def isNestedBuildAllowed(path: os.Path): Boolean = {
    try {
      val yamlHeader = mill.constants.Util.readBuildHeader(path.toNIO, path.last, true)
  ...
  def generateWrappedAndSupportSources(
    ...
    val nestedBuildRoots = scriptSources
      .collect {
        case path
            if buildFileNames.contains(path.last)
              && (path / os.up) != projectRoot
              && isNestedBuildAllowed(path) =>
          (path / os.up, packagePrefixFor(path))
      }
      .distinct

    def aliasImportsFor(path: os.Path): String = {
      val matchingRoot = nestedBuildRoots
        .filter { case (dir, _) => path.startsWith(dir) }
        .maxByOption(_._1.segments.size)

      matchingRoot match {
        case Some((_, pkg)) =>
          s"import _root_.${pkg}.{package_ => build}\n" +
            s"import _root_.${CGConst.globalPackagePrefix}.{package_ => buildRoot}"
        case None =>
          // Provide `build` as an alias to the root `build_.package_`. From the user's
          // perspective it looks like they're writing code in `package build`, but at
          // compile-time we rename things, so we provide this alias to preserve that fiction.
    ...
    for (scriptPath <- scriptSources) {
      ...
      val aliasImports = aliasImportsFor(scriptPath)
  ...
  private def generateBuildScript(
    ...
    val localBuildObject = if (isNestedBuildFile && childNames.nonEmpty) {
      val childRefs = childNames.map(c =>
        s"lazy val ${backtickWrap(c)} = ${CGConst.wrapperObjectName}.${backtickWrap(c)}"
      ).mkString("\n  ")
      s"""
         |object build {
         |  $childRefs
