...
trait ScalaNativeModule extends ScalaModule { outer =>
  ...
  override def transitiveIvyDeps: T[Agg[Dep]] =
    T {
  ...
  override def run(args: String*) =
    T.command {
      Jvm.baseInteractiveSubprocess(Vector(nativeLink().toString) ++ args, forkEnv(), workingDir = ammonite.ops.pwd)
    }
}
...
trait TestScalaNativeModule extends ScalaNativeModule with TestModule { testOuter =>
  ...
  override protected def testTask(args: Task[Seq[String]]): Task[(String, Seq[TestRunner.Result])] =
    T.task {
      ...
      val (doneMsg, results) = if (frameworks.nonEmpty && testsMap.nonEmpty) {
        ...
        val testClassloader =
          new URLClassLoader(runClasspath().map(_.path.toIO.toURI.toURL).toArray, this.getClass.getClassLoader)
        ...
        val nativeFrameworks = (cl: ClassLoader) =>
          frameworkInstances.zipWithIndex.map {
            case (f, id) =>
  ...
  private def frameworksAndTestsMap =
    T {
      val frameworkInstances = TestRunner.frameworks(testFrameworks()) _

      val testClasses = Jvm.inprocess(
        runClasspath().map(_.path),
