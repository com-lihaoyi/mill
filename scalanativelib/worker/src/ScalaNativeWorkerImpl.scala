...
class ScalaNativeWorkerImpl extends mill.scalanativelib.api.ScalaNativeWorkerApi {
  ...
  def discoverTarget(clang: java.io.File, workdir: java.io.File): String =
    Discover.targetTriple(clang.toPath, workdir.toPath)
  ...
  override def newScalaNativeFrameWork(
      framework: Framework,
      id: Int,
      testBinary: File,
      logLevel: NativeLogLevel,
