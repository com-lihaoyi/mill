...
4. `fast` runs without packaging code into jars and re-uses the `out/` folder the Mill daemon between runs.
...
that changed in that PR.

* CI uses `selectiveInputs` to customize the selective testing behavior for the following test jobs,
to only run them if the corresponding `libs/` module has changed, ignoring changes in the core Mill
codebase. This is a heuristic optimization that assumes that bugs in the core Mill code paths will
get caught in one of the other jobs.
...
* To disable selective test execution on a PR, you can label it with `run-all-tests`, which
