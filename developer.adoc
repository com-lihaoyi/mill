...
| Sub-Process Tests| `example.\\__.local.daemon`, `integration.__.local.daemon` | `dist.run` | `test-mill-dev.sh`
...
doesn't work you can use `mill.constants.DebugLog.println`/`mill.api.Debug` which writes to a file
...
In general, you use `example.basic[3-simple]` or `integration.feature[inspect]` to run subproxess
tests using the default configuration. You can reproduce these tests manually using
