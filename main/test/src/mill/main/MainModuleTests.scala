  ...
  override def tests: Tests = Tests {
    ...
    test("inspect") {
      test("single") - UnitTester(mainModule, null).scoped { eval =>
        val res =
          eval.evaluator.executeTasks(Seq(mainModule.inspect(eval.evaluator, "hello")))
      ...
      test("multi") - UnitTester(mainModule, null).scoped { eval =>
        val res =
          eval.evaluator.executeTasks(Seq(mainModule.inspect(
            eval.evaluator,
            "hello",
            "hello2"
          )))
    ...
    test("clean") {
      ...
      test("single-target") {
        ...
        val r2 =
          ev.evaluator.executeTasks(Seq(cleanModule.clean(ev.evaluator, "foo.target")))
        assert(r2.failing.size == 0)
        checkExists(false)(
    ...
    test("cleanWorker") {
      ...
      test("single-target") {
        ...
        val r2 = ev.evaluator.executeTasks(Seq(workerModule.clean(
          ev.evaluator,
          "foo.theWorker"
        )))
        assert(r2.failing.size == 0)
        ...
        val r3 = ev.evaluator.executeTasks(Seq(workerModule.clean(
          ev.evaluator,
          "bar.theWorker"
        )))
        ...
        val r4 = ev.evaluator.executeTasks(Seq(workerModule.clean(
          ev.evaluator,
          "bazz[1].theWorker"
        )))
      ...
      test("single-module") {
        ...
        val r4 =
          ev.evaluator.executeTasks(Seq(workerModule.clean(ev.evaluator, "bazz[1]")))
        assert(r4.failing.size == 0)
        assert(workers.size == 2)
      }
    }
