...
object RunScript {
  def runScript(
    ...
    } yield (evaluator, evalWatches, res.map(_.flatMap(_._2)))
  ...
  def evaluateRootModule(
    ...
    for {
      ...
      module <-
        try Util.withContextClassloader(interp.evalClassloader) {
          Res.Success(
            buildCls
              .getMethod("millSelf")
              .invoke(null)
          ...
          case (scopedSel, sel) =>
            for (res <- prepareResolve(evaluator, scopedSel, sel))
              yield {
                ...
                } finally mill.eval.Evaluator.currentEvaluator.set(null)
              }
        }
  ...
  def evaluate(
    ...
    evaluated.failing.keyCount match {
      case 0 =>
        val json = for (t <- targets.toSeq) yield t match {
