  ...
  def endResolveLabel(
    ...
    last match {
      ...
      case _ =>
        ...
        command
          .orElse(target)
          .orElse(Resolve.runDefault(obj, Segment.Label(last), discover, rest).flatten.headOption) match {
          case None =>
            Resolve.errorMsgLabel(
              singleModuleMeta(obj, discover, obj.millModuleSegments.value.isEmpty),
              Seq(Segment.Label(last)),
...
object Resolve {
  ...
  def errorMsgBase[T](direct: Seq[T], last0: T, editSplit: String => String, defaultErrorMsg: String)(
  ...
  def errorMsgCross(crossKeys: Seq[Seq[String]], last: Seq[String], revSelectorsSoFar: Seq[Segment]) =
    errorMsgBase(
      crossKeys,
      last,
      x => x,
  ...
  def invokeCommand(target: Module, name: String, discover: Discover[_], rest: Seq[String]) =
    for {
      (cls, entryPoints) <- discover.value
  ...
  def runDefault(obj: Module, last: Segment, discover: Discover[_], rest: Seq[String]) =
    for {
...
abstract class Resolve[R: ClassTag] {
  def endResolveCross(obj: Module, last: List[String], discover: Discover[_], rest: Seq[String]): Either[String, Seq[R]]
  def endResolveLabel(obj: Module, last: String, discover: Discover[_], rest: Seq[String]): Either[String, Seq[R]]
