...
object MainModule {
  def resolveTasks[T](evaluator: Evaluator, targets: Seq[String], multiSelect: Boolean)(f: List[NamedTask[Any]] => T) =
...
trait MainModule extends mill.Module {
  ...
  /**
    ...
    */
  def resolve(evaluator: Evaluator, targets: String*) =
    mill.T.command {
  ...
  /**
    ...
    */
  def plan(evaluator: Evaluator, targets: String*) =
    mill.T.command {
  ...
  /**
    ...
    * chosen is arbitrary.
    */
  def path(evaluator: Evaluator, src: String, dest: String) =
    mill.T.command {
      val resolved = RunScript.resolveTasks(
        mill.main.ResolveTasks,
        evaluator,
  ...
  /**
    ...
    */
  def inspect(evaluator: Evaluator, targets: String*) =
    mill.T.command {
  ...
  /**
    ...
    */
  def all(evaluator: Evaluator, targets: String*) =
    mill.T.command {
  ...
  /**
    ...
    */
  def par(evaluator: Evaluator, targets: String*) =
    mill.T.command {
      MainModule.evaluateTasks(evaluator = evaluator, targets = targets, multiSelect = true) { res =>
        res.flatMap(_._2)
      }
    }
  ...
  /**
    ...
    */
  def show(evaluator: Evaluator, targets: String*) =
    mill.T.command {
      MainModule.evaluateTasks(
        evaluator.copy(
  ...
  /**
    ...
    */
  def clean(evaluator: Evaluator, targets: String*) =
    mill.T.command {
      ...
      def keepPath(path: os.Path) =
        path.segments.toSeq.lastOption match {
  ...
  /**
    ...
    */
  def visualize(evaluator: Evaluator, targets: String*) =
    mill.T.command {
  ...
  /**
    ...
    */
  def visualizePlan(evaluator: Evaluator, targets: String*) =
    mill.T.command {
      plan0(evaluator, targets) match {
        case Left(err) => Result.Failure(err)
