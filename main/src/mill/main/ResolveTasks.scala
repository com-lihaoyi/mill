...
object ResolveTasks {
        ...
        head match {
          case Segment.Label(singleLabel) =>
            recurse(
              ...
              singleLabel match {
                ...
                case "__" =>
                  errorMsgLabel(
                    sys.error("singleModuleMeta(obj, discover, obj.millModuleSegments.value.isEmpty)"),
                    remainingSelector,
          ...
          case Segment.Cross(cross) =>
            obj match {
              ...
              case _ =>
                Left(
                  unableToResolve(Segment.Cross(cross.map(_.toString)), tail) +
                    hintListLabel(tail)
                )
            }
        }
  ...
  def endResolveCross(
    ...
    obj match {
      case _: Cross[Module] =>
        runDefault(obj, Segment.Cross(last), discover, rest).flatten.headOption match {
          case None =>
            Left(
      ...
      case _ =>
        Left(
          unableToResolve(Segment.Cross(last), obj.millModuleSegments.value) +
            hintListLabel(obj.millModuleSegments.value)
        )
    }
  }
