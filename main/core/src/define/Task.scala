...
object Target extends TargetGenerated with Applicative.Applyer[Task, Task, Result, mill.api.Ctx] {
  ...
  implicit def apply[T](t: T)(implicit rw: RW[T], ctx: mill.define.Ctx): Target[T] = macro targetImpl[T]

  def targetImpl[T: c.WeakTypeTag](
      c: Context
  )(t: c.Expr[T])(rw: c.Expr[RW[T]], ctx: c.Expr[mill.define.Ctx]): c.Expr[Target[T]] = {
    import c.universe._
  ...
  implicit def apply[T](t: Result[T])(implicit rw: RW[T], ctx: mill.define.Ctx): Target[T] = macro targetResultImpl[T]

  def targetResultImpl[T: c.WeakTypeTag](
      c: Context
  )(t: c.Expr[Result[T]])(rw: c.Expr[RW[T]], ctx: c.Expr[mill.define.Ctx]): c.Expr[Target[T]] = {
    import c.universe._
  ...
  def apply[T](t: Task[T])(implicit rw: RW[T], ctx: mill.define.Ctx): Target[T] = macro targetTaskImpl[T]

  def targetTaskImpl[T: c.WeakTypeTag](
      c: Context
  )(t: c.Expr[Task[T]])(rw: c.Expr[RW[T]], ctx: c.Expr[mill.define.Ctx]): c.Expr[Target[T]] = {
    import c.universe._
    mill.moduledefs.Cacher.impl0[Target[T]](c)(
      reify(
  ...
  def sourcesImpl1(c: Context)(values: c.Expr[Result[os.Path]]*)(ctx: c.Expr[mill.define.Ctx]): c.Expr[Sources] = {
    import c.universe._
    val wrapped =
      for (value <- values.toList)
        yield Applicative
          .impl0[Task, PathRef, mill.api.Ctx](c)(
  ...
  def sources(values: Result[Seq[PathRef]])(implicit ctx: mill.define.Ctx): Sources = macro sourcesImpl2

  def sourcesImpl2(c: Context)(values: c.Expr[Result[Seq[PathRef]]])(ctx: c.Expr[mill.define.Ctx]): c.Expr[Sources] = {
    import c.universe._
  ...
  def sourceImpl1(c: Context)(value: c.Expr[Result[os.Path]])(ctx: c.Expr[mill.define.Ctx]): c.Expr[Source] = {
    import c.universe._
  ...
  def sourceImpl2(c: Context)(value: c.Expr[Result[PathRef]])(ctx: c.Expr[mill.define.Ctx]): c.Expr[Source] = {
    import c.universe._
    mill.moduledefs.Cacher.impl0[Source](c)(
      reify(new Source(makeT(Nil, _ => value.splice), ctx.splice))
    )
  }
  def input[T](value: Result[T])(implicit rw: RW[T], ctx: mill.define.Ctx): Input[T] = macro inputImpl[T]

  def inputImpl[T: c.WeakTypeTag](
      c: Context
  )(value: c.Expr[T])(rw: c.Expr[RW[T]], ctx: c.Expr[mill.define.Ctx]): c.Expr[Input[T]] = {
    import c.universe._
  ...
  def command[T](
      t: Task[T]
  )(implicit ctx: mill.define.Ctx, w: W[T], cls: EnclosingClass, overrides: Overrides): Command[T] =
    new Command(t, ctx, w, cls.value, overrides.value)

  def command[T](
      t: Result[T]
  )(implicit w: W[T], ctx: mill.define.Ctx, cls: EnclosingClass, overrides: Overrides): Command[T] =
    macro commandImpl[T]
  ...
  def workerImpl1[T: c.WeakTypeTag](c: Context)(t: c.Expr[Task[T]])(ctx: c.Expr[mill.define.Ctx]): c.Expr[Worker[T]] = {
    import c.universe._
  ...
  def workerImpl2[T: c.WeakTypeTag](c: Context)(t: c.Expr[T])(ctx: c.Expr[mill.define.Ctx]): c.Expr[Worker[T]] = {
    import c.universe._
    mill.moduledefs.Cacher.impl0[Worker[T]](c)(
      reify(
  ...
  def persistent[T](t: Result[T])(implicit rw: RW[T], ctx: mill.define.Ctx): Persistent[T] = macro persistentImpl[T]

  def persistentImpl[T: c.WeakTypeTag](
      c: Context
  )(t: c.Expr[T])(rw: c.Expr[RW[T]], ctx: c.Expr[mill.define.Ctx]): c.Expr[Persistent[T]] = {
    import c.universe._
  ...
  type TT[+X] = Task[X]
  def makeT[X](inputs0: Seq[TT[_]], evaluate0: mill.api.Ctx => Result[X]) =
    new Task[X] {
      val inputs                    = inputs0
...
class Command[+T](t: Task[T], ctx0: mill.define.Ctx, val writer: W[_], val cls: Class[_], val overrides: Int)
    extends NamedTaskImpl[T](ctx0, t) {
...
class Persistent[+T](t: Task[T], ctx0: mill.define.Ctx, readWrite: RW[_]) extends TargetImpl[T](t, ctx0, readWrite) {
