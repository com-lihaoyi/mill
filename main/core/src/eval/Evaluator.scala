...
case class Evaluator(
  ...
  def parallelEvaluate(
    ...
    try {
      ...
      for (k <- terminals) {
        ...
        futures(k) = Future.sequence(deps.map(futures)).map { upstreamValues =>
          if (failed.get()) None
            ...
            val upstreamResults = upstreamValues.iterator
      ...
      val results = terminals.flatMap { t =>
  ...
  def handleTaskResult(v: Any, hashCode: Int, metaPath: os.Path, inputsHash: Int, labelledNamedTask: Labelled[_]) =
  ...
  def resolveLogger(logPath: Option[os.Path], logger: Logger): Logger =
    logPath match {
      case None       => logger
...
object Evaluator {
  ...
  def makeSegmentStrings(segments: Segments) =
    segments.value.flatMap {
