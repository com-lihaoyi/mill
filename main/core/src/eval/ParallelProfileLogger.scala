...
class ParallelProfileLogger(outPath: os.Path, startTime: Long) {
  ...
  def getThreadId(thread: String) =
    synchronized {
      threadIds.getOrElseUpdate(thread, threadIds.size)
    }
  def timeTrace(task: String, cat: String, startTime: Long, endTime: Long, thread: String, cached: Boolean): Unit =
    synchronized {
...
/**
  ...
  */
case class TraceEvent(name: String, cat: String, ph: String, ts: Long, dur: Long, pid: Int, tid: Int, args: Seq[String])
object TraceEvent {
