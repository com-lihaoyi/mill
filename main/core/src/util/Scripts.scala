...
object Scripts {
  ...
  def formatMainMethodSignature[T](base: T, main: Router.EntryPoint[T], leftIndent: Int, leftColWidth: Int): String = {
          ...
          if (duplicate.isEmpty) ""
            ...
            val lines =
              for ((sig, options) <- duplicate)
                yield s"Duplicate arguments for (--${sig.name}: ${sig.typeString}): " +
  ...
  def renderArg[T](base: T, arg: ArgSig[T, _], leftOffset: Int, wrappedWidth: Int): (String, String) = {
