...
object ParseArgs {
  ...
  private object BraceExpansionParser {
    ...
    def toExpand[_: P]: P[Fragment] =
      P("{" ~ braceParser.rep(1).rep(sep = ",") ~ "}").map(x => Fragment.Expand(x.toList.map(_.toList)))

    def braceParser[_: P]: P[Fragment] = P(toExpand | plainChars)

    def parser[_: P]: P[Seq[String]] =
      P(braceParser.rep(1).rep(sep = ",") ~ End).map { vss =>
  ...
  private def parseSelector(input: String): Parsed[(Option[Segments], Segments)] = {
    ...
    def simpleQuery[_: P] =
      P(segment ~ ("." ~ segment | crossSegment).rep).map {
        case (h, rest) => Segments(h :: rest.toList: _*)
      }
    def query[_: P] =
      P(simpleQuery ~ ("/" ~/ simpleQuery).?).map {
        case (q, None)     => (None, q)
