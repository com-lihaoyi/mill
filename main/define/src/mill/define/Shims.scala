...
object ShimService {
  ...
  def reflect(using Quotes): ShimService[quotes.type] =
    ...
    cls.getDeclaredConstructor(classOf[Quotes]).newInstance(summon[Quotes]).asInstanceOf[ShimService[quotes.type]]
  ...
  private class DottyInternal(val quotes: QuotesImpl) {
    ...
    def newClass(
      ...
      cls.enter(dotc.core.Symbols.newConstructor(cls, dotc.core.Flags.Synthetic, names.map(_.toTermName), argTpes))
