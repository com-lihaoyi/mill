...
object Target extends Applicative.Applyer[Task, Task, Result, mill.api.Ctx] {
   ...
   * since any invalidation would be handled by the runtime build graph evaluation
   * anyway, while still preserving other "something -> task" method calls e.g. those
   * by the trait method forwarders generated by the Scala compiler
   */
  @mill.api.internal
  def codeSigIgnoreSimpleTargetCalls(): Unit = ()
  ...
  object Internal {
    ...
    def targetImpl[T: c.WeakTypeTag](c: Context)(t: c.Expr[T])(
      ...
      mill.moduledefs.Cacher.impl0[Target[T]](c)(
        reify {
          Target.codeSigIgnoreSimpleTargetCalls()
          new TargetImpl[T](
            lhs.splice,
            ctx.splice,
            rw.splice,
