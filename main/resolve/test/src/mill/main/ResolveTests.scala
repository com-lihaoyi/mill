...
object ResolveTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    "single" - {
      ...
      "neg7" - check("", Left("Selector cannot be empty. Try `mill resolve _` to see what's available."))
    }
    "backtickIdentifiers" - {
      ...
      "neg4" - check("", Left("Selector cannot be empty. Try `mill resolve _` to see what's available."))
      "neg5" - check(
        "invisible",
