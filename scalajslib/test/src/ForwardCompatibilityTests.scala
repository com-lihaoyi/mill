...
// Analogous to mill.scalalib.ForwardCompatibilityTests
object ForwardCompatibilityTests extends TestSuite {
  trait ForwardCompat extends TestUtil.BaseModule {
    ...
    object foo extends ScalaJSModule with PublishModule {
      ...
      override def scalaJSVersion = "1.9.0"
    ...
    object bar extends ScalaJSModule {
  ...
  def tests: Tests = Tests {
    ...
    "run ivy-dependent project" - workspaceTest(ForwardCompat2) { eval => 
      val Right(_) = eval.apply(ForwardCompat2.foo.publishLocal())
