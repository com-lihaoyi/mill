...
object Lib {
  ...
  def scalaJSLinkerIvyDep(scalaJSVersion: String): Dep =
    Dep(
      "com.lihaoyi",
      s"mill-jsbridge_${scalaJSVersion.replace('.', '_')}",
      "0.1-SNAPSHOT"
    )

  def scalaJSLinkerBridge(classPath: Loose.Agg[Path]): ScalaJSLinkerBridge = {
    val classloaderSig =
      classPath.map(p => p.toString().hashCode + p.mtime.toMillis).sum
  ...
  def link(main: Option[String],
    ...
    if (inputPaths.nonEmpty) {
      mkdir(outputPath / up)
      val inputFiles: Vector[File] =
        inputPaths
          .map(ls)
          .flatMap(_.filter(_.ext == "sjsir"))
          .map(_.toIO)(breakOut)
      val inputLibraries: Vector[File] =
        libraries.filter(_.ext == "jar").map(_.toIO).toVector
