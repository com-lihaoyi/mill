...
you'll need to do this. Mill can do this for you via
...
This command requests some basic metadata necessary for generating the GPG keys,
generates a key-pair, registers it, and prints out instructions on how to use
it from the terminal or in CI.
...
* xref:#_manual_gpg_setup[Manual GPG Setup]

=== Publishing

Once you have the various secrets ``export``ed as above, you can publish all
...
- `bundleName`: All non-snapshot packages are uploaded in a single bundle. This option overrides that bundle's name. Recommended bundle name syntax: groupName-artifactId-versionNumber. A single package defaults to its group, artifact ID, and version. When publishing several packages, the default combines the project directory name with the version occurring most often among them. Characters other than letters, digits, `.`, `_`, and `-` in the project directory name are replaced with `-`. Snapshot packages are always uploaded separately.
...
If you have never used GPG before, xref:#_setting_up_gpg[initGpgKeys] is probably the easiest way to
get started, but you can also follow
https://central.sonatype.org/publish/requirements/gpg/[Sonatype's GPG Documentation]
which has manual instructions on how to do this.
