...
You can also use the `;force` suffix in the short dependency notation, which is equivalent to calling `.forceVersion()` and is usable from YAML build headers:

[,scala]
----
val deps = Seq(mvn"com.lihaoyi::fansi:0.2.14;force")
----
