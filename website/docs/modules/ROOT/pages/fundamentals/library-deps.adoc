...
To render a tree of dependencies (transitive included) you can run `mill myModule.mvnDepsTree`. Here is how the start of `./mill __.mvnDepsTree` looks like in the `mill` project itself:
...
After running the `mvnDepsTree` command you'll also find the `mvnDepsTree.json` and `mvnDepsTree.log` file that contain the output of the above `mvnDepsTree` command.
...
from. The output of `mvnDepsTree` can be quite large in larger projects so the
...
[source,txt]
----
❯ ./mill -i dev.run ~/Documents/scala-workspace/com-lihaoyi/mill  main.mvnDepsTree --whatDependsOn com.github.plokhotnyuk.jsoniter-scala:jsoniter-scala-core_2.13
[33/33] main.mvnDepsTree
