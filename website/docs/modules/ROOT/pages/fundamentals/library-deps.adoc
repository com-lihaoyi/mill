...
----
jvm"{organization}:{name}:{version}"
----

Additional attributes are also supported:

----
jvm"{organization}:{name}:{version}[;{attribute}={value}]*"
----
...
----
def jvmDeps = Seq(
  jvm"org.slf4j:slf4j-api:1.7.25"
)
----
...
If you need additional test dependencies, you simply add them by overriding `def jvmDeps`, as you would do with normal library dependencies.

When migrating a sbt project and seeing a dependency like this: `"ch.qos.logback" % "logback-classic" % "1.2.3" % "test"`,
simply add it to the test module's `jvmDeps` as ordinary dependency.
...
object main extends JavaModule {
  object test extends JavaTests {
    def jvmDeps = Seq(
      jvm"org.qos.logback:logback-classic:1.2.3"
    )
  }
}
----
...
If you want to use a dependency only at compile time, you can declare it with the `compileJvmDeps` task.
...
If you want to declare dependencies to be used at runtime (but not at compile time), you can use the `runJvmDeps` tasks.

.Example
[source,scala]
----
def runJvmDeps = Seq(
  jvm"ch.qos.logback:logback-classic:1.2.0"
)
----

It is also possible to use a higher version of the same library dependencies already defined in `jvmDeps`, to ensure you compile against a minimal API version, but actually run with the latest available version.

== Transitive dependencies

To render a tree of dependencies (transitive included) you can run `mill myModule.jvmDepsTree`. Here is how the start of `./mill __.jvmDepsTree` looks like in the `mill` project itself:
...
After compiling your module(s) you can find and examine files such as `jvmDeps.json` and `transitiveJvmDeps.json` in your `out` build's folder for a given module.
After running the `jvmDepsTree` command you'll also find the `jvmDepsTree.json` and `jvmDepsTree.log` file that contain the output of the above `jvmDepsTree` command.

You can observe the actual version being used by running `mill show myModule.resolvedJvmDeps`. If you run `mill myModule.resolvedJvmDeps`, the same information is available in `out/myModule/resolvedJvmDeps.json`.

=== Figuring out where a dependency comes from

There will be times when you want to figure out where a dependency is coming
from. The output of `jvmDepsTree` can be quite large in larger projects so the
...
[source,txt]
----
❯ ./mill -i dev.run ~/Documents/scala-workspace/com-lihaoyi/mill  main.jvmDepsTree --whatDependsOn com.github.plokhotnyuk.jsoniter-scala:jsoniter-scala-core_2.13
[33/33] main.jvmDepsTree
...
----
def deps = Seq(
  jvm"com.lihaoyi::pprint:0.5.3".exclude("com.lihaoyi" -> "fansi_2.12")
)
----
...
* You declare a dependency `val deps = Seq(jvm"com.lihaoyi::fansi:0.2.14")`
* There is another dependency, `val deps = Seq(jvm"com.lihaoyi::PPrint:0.8.1")`
* PPrint 0.8.1 uses fansi 0.4.0, so it is a transitive dependency
* `mill show myModule.resolvedJvmDeps | grep "fansi"` should show fansi 0.4.0
* If you want to force to the older version (to prevent it being evicted, and replaced by 0.4.0) then you can use `val deps = Seq(jvm"com.lihaoyi::fansi:0.2.14".forceVersion())`
* `mill show myModule.resolvedJvmDeps | grep "fansi"` should show fansi 0.2.14
...
* if its version is empty in an `jvmDeps` section in Mill, the version from dependency management will be used
