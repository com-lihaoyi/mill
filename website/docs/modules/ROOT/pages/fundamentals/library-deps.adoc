...
printed; later occurrences are marked with a trailing `(*)` and their subtree is left out. Without
that, the size of the tree would grow with the number of distinct _paths_ through the dependency
graph rather than with the number of dependencies, which for larger projects means it may never
finish printing.
...
[source,txt]
----
❯ ./mill dev.run ~/Documents/scala-workspace/com-lihaoyi/mill  main.showMvnDepsTree --whatDependsOn com.github.plokhotnyuk.jsoniter-scala:jsoniter-scala-core_2.13
