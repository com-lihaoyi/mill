...
If you need additional test dependencies, you simply add them by overriding `def libraryDeps`, as you would do with normal library dependencies.

When migrating a sbt project and seeing a dependency like this: `"ch.qos.logback" % "logback-classic" % "1.2.3" % "test"`,
simply add it to the test module's `libraryDeps` as ordinary dependency.
...
If you want to use a dependency only at compile time, you can declare it with the `compileLibraryDeps` task.
...
If you want to declare dependencies to be used at runtime (but not at compile time), you can use the `runLibraryDeps` tasks.
...
It is also possible to use a higher version of the same library dependencies already defined in `libraryDeps`, to ensure you compile against a minimal API version, but actually run with the latest available version.

== Transitive dependencies

To render a tree of dependencies (transitive included) you can run `mill myModule.libraryDepsTree`. Here is how the start of `./mill __.libraryDepsTree` looks like in the `mill` project itself:
...
After compiling your module(s) you can find and examine files such as `libraryDeps.json` and `transitiveLibraryDeps.json` in your `out` build's folder for a given module.
After running the `libraryDepsTree` command you'll also find the `libraryDepsTree.json` and `libraryDepsTree.log` file that contain the output of the above `libraryDepsTree` command.

You can observe the actual version being used by running `mill show myModule.resolvedLibraryDeps`. If you run `mill myModule.resolvedLibraryDeps`, the same information is available in `out/myModule/resolvedLibraryDeps.json`.

=== Figuring out where a dependency comes from

There will be times when you want to figure out where a dependency is coming
from. The output of `libraryDepsTree` can be quite large in larger projects so the
...
[source,txt]
----
❯ ./mill -i dev.run ~/Documents/scala-workspace/com-lihaoyi/mill  main.libraryDepsTree --whatDependsOn com.github.plokhotnyuk.jsoniter-scala:jsoniter-scala-core_2.13
[33/33] main.libraryDepsTree
...
* You declare a dependency `val deps = Seq(ivy"com.lihaoyi::fansi:0.2.14")`
* There is another dependency, `val deps = Seq(ivy"com.lihaoyi::PPrint:0.8.1")`
* PPrint 0.8.1 uses fansi 0.4.0, so it is a transitive dependency
* `mill show myModule.resolvedLibraryDeps | grep "fansi"` should show fansi 0.4.0
* If you want to force to the older version (to prevent it being evicted, and replaced by 0.4.0) then you can use `val deps = Seq(ivy"com.lihaoyi::fansi:0.2.14".forceVersion())`
* `mill show myModule.resolvedLibraryDeps | grep "fansi"` should show fansi 0.2.14
...
* if its version is empty in an `libraryDeps` section in Mill, the version from dependency management will be used
