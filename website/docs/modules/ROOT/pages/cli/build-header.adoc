...
Alternatively, you can use a `build.mill.yaml` file to specify the JVM version as a top-level YAML key:

include::partial$example/cli/header/2-jvm-version-yaml.adoc[]
...
xref:javalib/module-config.adoc#_configuring_single_file_scripts[Configuring Single-File Scripts],
