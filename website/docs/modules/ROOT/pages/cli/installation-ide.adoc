...
Mill also supports defining the Mill version in an attribute of the YAML-header at
the top of your `build.mill` file:
...
Mill uses a JVM-based xref:javalib/publishing.adoc#_building_executable_assemblies_with_assemblymodule[Assembly Jar],
as the launcher on On Windows-ARM, and you can enable this on other platforms
by adding a a `-jvm` suffix on the Mill version:

[source,console,subs="verbatim,attributes"]
----
> echo {version}-jvm > .mill-version
----
...
via a  `.mill-jvm-version`, which also becomes the default version used for any JVMs spawned
by Mill to `run` or `test` your project:
...
`.mill-jvm-version` can help ensure your project uses a consistent JVM version and
...
Note that you may need to `./mill shutdown` before calling Mill with the environment
variable to make sure it gets properly propagated to Mill's background daemon.
