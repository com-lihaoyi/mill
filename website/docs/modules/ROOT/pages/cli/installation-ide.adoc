...
[source,console,subs="verbatim,attributes"]
----
> sudo curl -L {mill-download-url}/mill-dist-{mill-version}-mill.sh -o /usr/local/bin/mill
...
xref:javalib/packaging.adoc#_building_native_image_binaries_with_graal_vm[Graal Native Image],
...
when running Mill from the command line, and requires a JVM installed globally (version 17 or above)
in order to run.
...
This unpacks the completion script in `~/.local/share/mill/completion/mill-completion.sh`
and registers it in your `~/.bash_profile` and `~/.zshrc`. After that, doing `./mill <TAB>`
should begin showing possible modules and tasks for you to select at the command line:
...
When running Mill in BSP mode, BSP now shares the regular `out/` directory and
daemon by default. This lets IDE-triggered requests reuse the same task caches
and concurrency-aware output locking as normal `./mill ...` commands.

If you want BSP to use its own dedicated output directory instead, set
`mill-separate-bsp-output-dir: true` in the root build header. This uses the
conventional `.bsp/out/` directory. For a custom dedicated BSP output path, set
the `MILL_BSP_OUTPUT_DIR` environment variable instead.
...
Mill's BSP IDE integration writes to log files under the selected BSP output
directory in `mill-bsp/`, for example `out/mill-bsp/` by default or
`.bsp/out/mill-bsp/` when `mill-separate-bsp-output-dir: true` is set. It
contains regular Mill output accompanied by additional BSP client-server
communication details. This can be useful to look at if your IDE fails to
import your Mill project.
...
To minimize the number of unnecessary tests that you run on CI jobs, you should
use xref:large/selective-execution.adoc[Selective Test Execution] for any large multi-module
codebase. This allows you to limit the tests that run on your codebase to those in modules
downstream of the code you changed, allowing you to save time and resources by not wastefully
running tests that could not possibly be affected by your code change.

=== Uploading Chrome Profiles

For investigating performance issues in CI, you may find it useful to upload the
xref:fundamentals/out-dir.adoc#_mill_chrome_profile_json[out/mill-chrome-profile.json] file
at the end of every run. This gives you insight into how long each task took, what tasks
were running sequentially or in paralle, and overall system CPU utilization during the run.

For example, the Github Actions steps to upload the `out/mill-chrome-profile.json` file as
an artifact looks something like this, placed after your `./mill` commands:
...
```yaml
  - name: Upload Chrome Profile
    ...
    if: always()
    with:
      name: mill-chrome-profile
      path: out/mill-chrome-profile.json
      if-no-files-found: ignore
```

You can then download the `mill-chrome-profile.zip` from the Github Actions Web UI, unzip
it, then load it into `chrome://tracing` to visualize the time spent during that CI run.
...
The recommended way to manage your Mill version is to rely on the `DEFAULT_MILL_VERSION` defined
in your `./mill` and `./mill.bat` bootstrap scripts. These can be updated via the builtin command
`updateMillScripts`:

```console
> ./mill updateMillScripts 1.1.0
```

In general, managing your Mill version via your `./mill` bootstrap scripts is most reliable, as
it ensures the bootstrap script and the Mill version you use are always in sync. However, Mill
also provides other ways of overriding the Mill version used, which we will discuss below:

=== Overriding your Mill Version

Apart from the version defined in your bootstrap script, you
can also override the Mill version via use a `mill-version` key in your xref:cli/build-header.adoc[]
or a `.mill-version` file. The file path `.config/mill-version` is also supported. If neither is provided,
...
In general, we recommend you manage your Mill version via your `./mill` bootstrap scripts and
update it via `./mill updateMillScripts`. That is the most reliable approach, and avoids issues
that may be caused by incompatibilities between your `./mill` script and the `mill-version`
you override.
...
You can update your `.mill-version` to the unstable version and the bootstrap script
...
No other installation methods of Mill are officially supported. Homebrew, Pacman, Scoop,
Docker, Coursier, etc. are maintained by third parties and cannot be guaranteed to work.
Please use the bootstrap script installation described on this page. If you have issues
with installing Mill from other package managers, please open an issue on their respective
codebase
