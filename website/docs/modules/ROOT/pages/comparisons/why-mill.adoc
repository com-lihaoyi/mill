...
Mill provides 3 major advantages over other JVM build tools. The comparison pages
...
Once we have run `//| mvnDeps:`, we can import `TemplateEngine`, `Context`, and replace our
...
project are complex. Every build tool needs the user define templated, customizable
graph-computations when setting up their build pipelines.

- “Computations” as you need to specify the build steps to be run
- “Graph” as the tool needs to order and parallelize steps in your build
- “Templated” as different modules in a build are often very similar
- “Customizable” as there are always some modules special in some way

For example, even a simple three-module Java build pipeline may look like this,
with many tasks doing different things, multiple modules with similar task layouts,
but with subtle customizations each module to wire them up:
...
inevitably end up being unfamiliar and confusing to users. Some build tools hide
this behind plugins, but end-users
still need to deal with these concepts when they are configuring or debugging their plugins.
Fundamentally, defining an API for templated, customizable
graph-computations is non-trivial, so is not surprising the APIs and to do so can get complicated.

Mill has the same requirement of defining templated, customizable graph-computations, but
rather than inventing a bespoke programming model to do so, Mill builds upon what everyone
already knows: Object-Oriented Programming. It turns out that the object-oriented
Java programmer knows provides all the key building
blocks necessary to define templated, customizable graph-computation:

- _Methods_ provide a way of defining discrete blocks of computation that perform
  some necessary action in your build

- The _Method Call Graph_ provides the build graph, and which methods call other methods
  defines which build steps require the output of other build steps

- _Classes_ provide the templating, where you can define a set of methods calling each
  other, and instantiate it more than once in different parts of your build

- _Overrides_ provide customization: where a particular instance or sub-class needs a
  different implementation of a build step, or even an entire portion of the build graph,
  you can override one-or-more methods to customize the method call graph to your liking
