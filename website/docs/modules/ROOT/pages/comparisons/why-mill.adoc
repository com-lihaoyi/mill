...
`object foo` into a `interface MyJavaModule` (called a `trait MyJavaModule` in Mill's syntax), so it
can be inherited
by `object foo` and `object bar` to share the configuration:
...
you can as xref:extending/writing-plugins.adoc#_publishing[publish
