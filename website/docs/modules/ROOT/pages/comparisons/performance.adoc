...
only to languages like C/C++/Rust, and xref:blog::1-java-compile.adoc[the Java compiler is extremely fast].
Mill is able to trim a lot of the overhead that other build tools like Maven or Gradle
impose, resulting in edit-compile-test workflows that are 3-7x faster than those of other
Java build tools.

To get a feel for this difference, play recording below to see Mill
(Left) and Maven (Right) running side by side compiling the Netty codebase.

:cast-file: ../_images/basic/MillMavenComparison.cast

include::partial$Asciinema_Comparison.adoc[]

Apart from the raw performance, Mill also has a lot of useful
performance-related features that you can use to help speed up your local development
and CI workflows to keep your development efforts productive. This article contains
a discussion of these performance differences, how the benchmarks were run, and
how you can try them out yourselves.

== Mill vs Maven

To compare Mill vs Maven, we took the https://github.com/netty/netty[Netty] project
...
equivalent builds to compare the performance: compiling everything, compiling a single module, etc.

Overall across our benchmarks, Mill is 4-7x faster than Maven for clean compiles,
both parallel and sequential, and for many modules or for a single module:

[cols="2,1,1,1"]
|===
| Benchmark | Maven | Mill | Speedup
| Sequential Clean Compile All | 98.78s | 24.43s | 4.0x
| Parallel Clean Compile All | 50.67s | 10.35s | 4.9x
| Clean Compile Single Module | 6.33s | 0.90s | 7.0x
| Incremental Compile Single Module | 6.76s | 0.22s | 30.7x
| No-Op Compile Single Module | 5.28s | 0.12s  | 44.0x
|===
...
Looking at these numbers, it is not surprising that the Java platform has a reputation for
being slow and clunky. Even in the _No-Op Compile Single Module_ case, it takes ~5 seconds
for Maven to realize there's nothing to do!
...
If Java developers had this kind of fast turnaround working on their code, no doubt Java
would have a very different reputation than it has today.

For more details on these benchmarks, including steps to reproduce them,
see xref:#_mill_vs_maven_in_depth[]

== Mill vs Gradle

We used the https://github.com/mockito/mockito[Mockito] codebase as the case study of Mill vs Gradle.
...
| Sequential Clean Compile All | 18.3s | 5.0s | 3.7x
| Parallel Clean Compile All | 14.8s | 3.87s | 3.6x
| Clean Compile Single Module | 4.65s | 0.89s | 5.2x
| Incremental Compile Single Module | 1.61s | 0.28s | 5.9x
| No-Op Compile Single Module | 1.02s | 0.10s | 10.2x
|===

Mill's various "clean compile" workflows 3-5x faster than Gradle's, while it's incremental
...
For more details on these Gradle benchmarks, with steps to reproduce them,
see xref:#_mill_vs_gradle_in_depth[]
...
The following sections will go deeper into the benchmarks, exploring the root cause of the
performance differences and what commands were used for each benchmark.

The benchmarks on these page were run on with Mill 1.1.0 on a 10CPU/32GB 2021 M1 Macbook Pro,
with whatever version of Maven or Gradle each respective project defaults to. Each command
was run manually 5 times to allow warmup then the last 3 times recorded. The Netty
and Mockito codebases used in the benchmark are available for download below, along with
their example Mill configuration, so you can try running the benchmark commands yourself.

Note that as Mill runs a JVM background daemon, it may require a few runs for the JVM to warm
up to peak performance, and do be aware that benchmarked performance is expected to differ
on different operating systems and hardware.
...
- https://repo1.maven.org/maven2/com/lihaoyi/mill-dist/{mill-version}/mill-dist-{mill-version}-example-thirdparty-netty.zip
...
equivalent Maven workflow. In most cases,  Mill is 4-7x faster than Maven. Below, we
...
[source,console]
----
$ ./mvnw clean; time ./mvnw -Pfast -Denforcer.skip=true install
97.20s
98.35s
100.8s

$ ./mill clean _; time ./mill -j1 __.compile
25.65s
23.63s
24.02s
----
...
The first thing to take note is that we use `./mill clean _` with an underscore `_`
rather than `./mill clean` for this benchmark. The difference is that `./mill clean _`
cleans the output of all user-defined modules or tasks, while `./mill clean` also
wipes out Mill's internal workers and machinery.

From a user perspective, `./mill clean _` more accurately reflects what you are likely to
see when working with your project, e.g. after performing a large refactor or git branch update
that forces everything to re-compile, so that is what we use in this benchmark

==== Maven Compile vs Install

The second thing you may notice is the use of `./mvnw install` rather than `./mvnw compile`.
The reason for this is that Maven's main mechanism for managing inter-module dependencies
is via the local artifact cache at `~/.m2/repository`. Although many workflows work with
`compile`, some don't. In particular, `./mvnw clean compile` on the Netty codebase fails with:
...
[source,console]
----
$ ./mill clean _; time ./mill -j1 __.jar
27.92s
27.85s
29.01s
----
...
[source,console]
----
$ ./mvnw clean; time ./mvnw -T 10 -Pfast -Denforcer.skip=true install
51.48s
49.17s
51.36s

$ ./mill clean _; time ./mill __.compile
10.73s
10.05s
10.27s
----
...
[source,console]
----
$ ./mvnw clean; time ./mvnw -pl common -Pfast -Denforcer.skip=true -Dmaven.test.skip=true install
6.38s
6.46s
6.17s
...
[source,console]
----
$ time ./mvnw -pl common -Pfast -Denforcer.skip=true install
6.31s
4.76s
4.77s
...
- https://repo1.maven.org/maven2/com/lihaoyi/mill-dist/{mill-version}/mill-dist-{mill-version}-example-thirdparty-mockito.zip

And run the commands listed in the below sections yourself. Note that as Mill runs a JVM
...
[source,console]
----
$ # edit gradle.properties to set org.gradle.parallel=false
...
`org.gradle.parallel=false` in `gradle.properties`, and pass `-j1` to limit Mill to a
single thread.
...
[source,console]
----
$ # edit gradle.properties to set org.gradle.parallel=true
$ ./gradlew clean; time ./gradlew classes testClasses --no-build-cache
14.1s
15.6s
14.8s
...
parallelism: Gradle by default, Mill without `-j1` to run on 10 cores (the number on my Macbook Pro).
...
This indicates the speedup Mill provides over Gradle is unrelated to the parallelism of
each tool.
...
$ ./mill clean _; time ./mill compile
0.84s
0.81s
1.03s
----
...
The Mill, Gradle, and Maven builds we discussed in this case study do the same thing: they
...
be a realistic comparison.

But the bottom line is that, building projects with Mill is significantly faster than
...
JVM development process, you should try Mill to see if it can give you a very different experience!
