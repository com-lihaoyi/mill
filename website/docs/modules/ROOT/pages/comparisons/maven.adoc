...
his page discusses some of the key ways that Mill both learns from Maven, and also
how Mill tries to improve upon some of its rough edges to provide a user experience
that lets the idea of _"Declarative Builds"_ really shine through.
...
and utilities in Java rather than Python or Bash, especially where the Java ecosystem
has high-quality libraries you can use (such as Jsoup above). But in practice, setting
up your single-file Java program to actually be _run_ proves quite a hurdle.
...
run on your colleague's Windows-11 desktop machine, or your Linux backend servers?
...
Similar to the challenges of xref:#_installing_java[], it can be non-trivial to ensure you are
using the same version of `mvn` on your Mac laptop, or your colleague's Windows desktop, and
on your Linux CI machines. Not impossible, but certainly fiddly and error-prone. And it's
another thing you have to maintain as the different development environments inevitable evolve
over time.
...
in Python, Javascript, or some other language instead even though the Java language
and library ecosystem are perfectly adequate.
...
It may even be preferable for it to be in Java when the time comes to add parallelism
and concurrency and other features that may be harder to implement in Python or Bash.
The problem here is really that Java _build tools_ make it hard to run the code you have
already written!
...
it is used regardless of where the project is being built. With Mill, the Java version you
use is managed declaratively: just like how Maven lets you declaratively configure the libraries
that a project needs, Mill lets you set the `mill-jvm-version` the project needs and then
handles any downloading and caching of those JVMs automatically for you.
...
global installation, and you can get started on Mill with just the bootstrap script
and nothing else.
...
and the other keys tell us configuration on that module we want to override
(in this case `mvnDeps`).
...
xref:javalib/config.adoc[]. However, as Mill follows Mavens idea of
_Convention over Configuration_, the  `build.mill.yaml` file only needs to contain
...
Although `mvn` using a different the command line syntax to run your Java code isn't a blocker,
it is another small hurdle on top of the many other hurdles that a new user faces when trying
to run their Java code. Mill tries to follow the common command-line conventions used by most
other programming languages, so that anyone who picks up a Mill project coming from other
languages or ecosystems will feel right at home.
...
really shines through in the user experience. We have seen similar approaches taken in
Python with https://docs.astral.sh/uv/guides/scripts/[uv run], and Mill brings that
approach to Java and the JVM ecosystem letting you run single-file scripts written
in Java, Scala, or Kotlin without the overhead of setting up a separate build file.
...
command-line. None of these are blockers, but each of them is a hurdle enough to make
someone re-consider whether they want to write their code in Java or whether they
should instead switch to Python or some other language.
...
zero-setup installation process and declarative `mill-jvm-version` management,
...
through to provide an experience that is every bit as smooth as running a Python
or Bash script from the command line.
