...
and any Mill project can set `jvmId` to specify what Java version to use and ensure
it is used regardless of where the project is being built. You can also have different
...
that a project needs such as `org.jsoup:jsoup:1.17.2`, Mill lets you set the `jvmId` the
project needs and then handles any downloading and caching of those JVMs automatically for you.
...
any Mill project you check out should be buildable and runnable using `./mill` without needing
any global installation, and you can get started on Mill with just the bootstrap script
and nothing else.
...
small hurdle on top of the many other hurdles that a new user faces when trying
...
really shines through in the user experience. Mill makes it super easy to configure
and run small Java projects with its minimal `build.mill.yaml`, and even easier to
run single-file scripts with it's `//|` build headers, while still supporting more
advanced use cases such as multi-module projects with custom build pipelines for
when your small project inevitably grows large
