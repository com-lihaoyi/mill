...
you can omit the scala version and instead use a double colon (`::`) between the `organization` and the `name`, e.g. `jvm"com.typesafe.akka:akka-actor_2.12:2.5.25"`.
...
you can use three colons (`:::`) between `organization` and `name`, e.g.: `jvm"org.scalamacros:::paradise:2.1.1"`.
...
def jvmDeps = Seq(
  ...
  jvm"com.typesafe.akka:akka-actor_2.12:2.5.25",
  jvm"com.typesafe.akka::akka-actor:2.5.25",
  jvm"org.scalamacros:::paradise:2.1.1"
)
----
...
def jvmDeps = Seq(
  jvm"com.lihaoyi::upickle:2.0.0".withDottyCompat(scalaVersion()) // <1>
)
----
