...
import de.tobiasroeser.mill.aspectj.*
...
import mill.*, mill.scalalib.*, mill.scalalib.scalafmt.*
...
[source,console]
----
> mill __.undeclaredCompileDependencies       # find all undeclared dependencies
> mill __.undeclaredCompileDependenciesTest   # fail if there are undeclared dependencies
> mill __.unusedCompileDependencies           # find all unused dependencies
> mill __.unusedCompileDependenciesTest       # fail if there are unused dependencies
----
...
It supports Mill versions 0.9.X and 0.10.X.
...
import de.tobiasroeser.mill.integrationtest.*
...
import mill.*
import de.tobiasroeser.mill.jbake.*
...
Official Openapitools Mill plugin for generating source files from an OpenApi spec.

Project home: https://github.com/OpenAPITools/openapi-generator/
...
object `package` extends JavaModule with MavenModule with OpenApiModule { // <3>
  ...
  object openapi extends OpenApiConfig { // <4>
    ...
    def sourceFolder: T[String] = "src/main/java" // <5>
  ...
  override def generatedSources: T[Seq[PathRef]] = Seq( // <6>
    PathRef(Task.dest),
    openapi.generate())
}
----
<1> load dependency to build-script
<2> import namespace
<3> mark your module as `OpenApiModule`
<4> configure a submodule where the generated artifacts will be produced
<5> define the source-root folder. This is needed because different generators use different defaults, and otherwise we cannot create the correct folder for `generatedSources`.
<6> attach the previously defined object to the compile phase by overwriting `generatedSources`
...
----
Here, `openapi` refers to the object defined previously.

For all defined options and tasks, please see the
https://github.com/OpenAPITools/openapi-generator/tree/master/modules/openapi-generator-mill-plugin[official documentation].
...
import mill.*, mill.scalalib.*
import de.tobiasroeser.mill.osgi.*
...
import io.github.hoangmaihuy.scalatsi.*
...
Mutation testing introduces changes to your code, then runs your unit tests against the changed code. It is expected that your unit tests will now fail. If they don't fail, it might indicate your tests do not sufficiently cover the code. It is similar to code coverage, but instead of just measuring which lines of code are executed, it measures what code is actually tested.

Project home: https://stryker-mutator.io

Source: https://github.com/stryker-mutator/stryker4s

.`build.mill`
[source,scala,subs="attributes,verbatim"]
----
//| mvnDeps: ["io.stryker-mutator::mill-stryker4s::1.0.2"]

import mill.*
import mill.scalalib.*
import stryker4s.mill.Stryker4sModule

object app extends ScalaModule with Stryker4sModule {
  // All settings are configurable and prefixed with `stryker`
  def strykerExcludedMutations = Some(Seq("BooleanLiteral", "StringLiteral"))
  def strykerReporters = Some(Seq("html", "json", "console"))
