...
A plugin that checks that `jvmDeps` and `ivyCompileDeps` accurately reflect the direct dependencies of your source code.
...
Found undeclared dependencies: (add these to jvmDeps)
    jvm"org.typelevel::cats-kernel:2.7.0",

Found unimported dependencies: (remove these from jvmDeps)
    jvm"org.typelevel::cats-effect:3.3.6",
...
object main extends ScalaModule with Mima {
  ...
  def mimaPreviousArtifacts = Seq(
    jvm"my_group_id::main:my_previous_version"
  )
...
It will run ScalablyTyped and add the converted dependencies to the module's `jvmDeps`.
