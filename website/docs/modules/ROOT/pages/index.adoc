...
* *Ergonomics*: Mill's xref:javalib/intro.adoc#_declarative_configuration[declarative config files]
are concise and easy to read, its xref:javalib/intro.adoc#_programmable_configuration[programmable config files]
have xref:cli/installation-ide.adoc#_ide_support[full IDE support in IntelliJ and VSCode], and its
xref:cli/installation-ide.adoc#_bashzsh_tab_completion[terminal tab-completion]
and syntax highlighted command-line output and error messages mean developers can easily
navigate their builds and resolve issues.
...
compiling the same Netty codebase with Mill (~7.5s) vs Maven (~55s)
+
...
building xref:javalib/packaging.adoc#_building_executable_assemblies[executable assemblies],
xref:javalib/packaging.adoc#_building_native_image_binaries_with_graal_vm[native binaries]
or xref:javalib/packaging.adoc#_java_installers_using_jpackage[installers],
