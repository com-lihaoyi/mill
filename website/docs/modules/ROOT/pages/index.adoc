...
* **Simpler than Maven**, with xref:javalib/config.adoc[build.mill.yaml files]
  often 1/10th the lines of an equivalent `pom.xml`

* **Easier than Gradle**, with
  xref:comparisons/why-mill.adoc#_object_oriented_builds[object-oriented builds]
  that any Java developer can intuitively understand

* **Fastest builds on the JVM**, xref:comparisons/why-mill.adoc#_performance[3-6x
