...
== Declarative (YAML) Android Build

include::partial$example/androidlib/kotlin/1-hello-kotlin-declarative.adoc[]

This is the same "Hello World" application as above, but configured declaratively via
`build.mill.yaml`/`package.mill.yaml` files instead of a Scala `build.mill`.
