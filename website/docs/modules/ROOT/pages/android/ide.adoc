...
- Run `mill mill.idea/` to generate the IntelliJ project files.
- Open the project in IntelliJ (File -> Open) and select the project directory.

As mentioned in the xref:cli/installation-ide.adoc#_intellij_idea_xml_support[IDE setup page], if you make any changes to your `build.mill` definitions, you must regenerate the IntelliJ project files by running `mill mill.idea/` again.
...
- If using IntelliJ project files but the project structure still seems broken, close the IDE and try repeating the instruction steps <<_generating_intellij_idea_project_files, above>> to regenerate the project files and re-import.
