...
== 5. Run your application on an Android Virtual Device (AVD)
...
You can provide additional arguments to the emulator command found in the https://developer.android.com/studio/run/emulator-commandline[official documentation], by adding them after the command.
If you want to exclude the default arguments provided by Mill, you can set the `--excludeDefaultArgs` flag.
[source,console]
----
...
> ./mill app.startAndroidEmulator -noaudio

# Exclude default arguments
> ./mill app.startAndroidEmulator --excludeDefaultArgs -avd test_avd
----

[CAUTION]
====
If setting the `--excludeDefaultArgs` flag, make sure to provide it first before any additional arguments.
====

=== Install and run your application on the running emulator
The `androidInstall` command installs the apk built by the `app.androidApk` command.
[source,console]
----
> ./mill app.androidInstall
----

The `run` command installs the apk and also starts the main activity of the application.
[source,console]
----
> ./mill app.run
----

If you want to start a specific activity, you can use the `androidRun` command with the `--activity` flag by providing the fully qualified name of the activity. If `--activity` is not provided, it defaults to the main activity detected from the manifest file.
[source,console]
----
> ./mill app.androidRun --activity com.example.MainActivity
----
...
=== Delete the AVD
When you no longer need the AVD, you can delete it using the following command:
[source,console]
----
> ./mill app.deleteAndroidVirtualDevice
---- 