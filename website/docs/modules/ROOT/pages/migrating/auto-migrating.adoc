...
`mill init` also detects common application frameworks and wires up the corresponding Mill module: xref:javalib/spring-boot.adoc[Spring Boot] projects are configured with `SpringBootModule`, and xref:javalib/web-examples.adoc#_quarkus_example[Quarkus] projects with `QuarkusModule`.
