...
** Third-party dependencies need to be translated into Mill's `def jvmDeps`
...
* Define the third-party dependencies for each module with `def jvmDeps`.
...
[source,scala]
----
def jvmDeps = Seq(jvm"com.google.guava:guava:3.3.1-jre")
----
|===
...
[source,scala]
----
def jvmDeps = Seq(jvm"com.lihaoyi::scalatags:0.12.0")
----
|===

* Again, test-only third-party dependencies are defined inside the `object test` submodule.

* Compile-only dependencies can be defined with `def compileJvmDeps`, and runtime-only/provided
  dependencies defined with `def runJvmDeps`
