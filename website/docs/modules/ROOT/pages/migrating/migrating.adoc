...
** Third-party dependencies need to be translated into Mill's `def libraryDeps`
...
* Define the third-party dependencies for each module with `def libraryDeps`.
...
[source,scala]
----
def libraryDeps = Seq(ivy"com.google.guava:guava:3.3.1-jre")
----
|===
...
[source,scala]
----
def libraryDeps = Seq(ivy"com.lihaoyi::scalatags:0.12.0")
----
|===

* Again, test-only third-party dependencies are defined inside the `object test` submodule.

* Compile-only dependencies can be defined with `def compileLibraryDeps`, and runtime-only/provided
  dependencies defined with `def runLibraryDeps`
