...
faster and easier to use build system are significant (see xref:comparisons/maven.adoc[],
xref:comparisons/gradle.adoc[], xref:comparisons/performance.adoc[]),
...
* Some modules may require specific xref:javalib/module-config.adoc#_basic_module_configuration[Compilation & Execution Flags]
...
* def may need to use frameworks like xref:javalib/spring-boot.adoc[Spring Boot]
...
the benefits of Mill outweighing the cost of migration,
