...
- *You must use the same Operating System and Filesystem*, as differences there will
...
Use `selective.resolveTree` before `selective.run`: this will print out what it was going to run,
why they are being run (e.g. due to an input task change, build code change, or Mill version change)
...
include::partial$example/large/selective/10-selective-inputs.adoc[]

== Further Reading

See the following blog post for a broader discussion on the idea of selective test
execution, beyond the Mill build tool:

- xref:blog::3-selective-testing.adoc[Faster CI with Selective Testing]