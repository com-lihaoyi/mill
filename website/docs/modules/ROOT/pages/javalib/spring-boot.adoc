...
This page contains examples of providing Mill as a build tool for Spring-Boot projects,
adapting the official Spring Boot documentation in increasing complexity.

In general, Spring-Boot projects are just Java projects, and work out of the box with Mill
once configured with the proper dependencies. These examples below show some build setups
common in spring boot projects - application code, test suites, integration tests, etc. -
but the rest of the documentation for xref:javalib/intro.adoc[Building Java with Mill] all
still applies.

SpringBootModule aims for an out of the box experience for Spring Boot projects. It provides a lot of features and sensible defaults (such as setting `-parameters` for javac options (`mandatoryJavacOptions`) and managing dependencies with `org.springframework.boot:spring-boot-dependencies` in the `bomMvnDeps`). It also allows you to customise and configure it as needed.

The examples below show some of the features and configuration options available for Spring Boot projects.

== Spring Boot Hello World App

include::partial$example/javalib/springboot/1-hello-spring-boot.adoc[]

include::partial$example/javalib/springboot/2-hello-spring-boot-programmable.adoc[]


== Spring Boot TodoMvc App


include::partial$example/javalib/springboot/3-todo-spring-boot.adoc[]

== Spring Boot Web with Initializr

include::partial$example/javalib/springboot/4-web-initializr.adoc[]

== Spring Reactive Web, Websocket example with Initializr

include::partial$example/javalib/springboot/5-web-socket-initializr.adoc[]

== Spring Boot with AOT processing and Native Image Support

include::partial$example/javalib/springboot/6-native.adoc[]
...
* xref:{language-small}lib/packaging.adoc#_building_repackage_assemblies[`RepackageModule`] - Creates self-running application assemblies in a single-jar
