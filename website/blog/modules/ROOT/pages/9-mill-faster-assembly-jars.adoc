...
Although building an assembly "clean" is just as slow as the one-step assembly creation,
as is the case where you change third party dependencies, in practice these scenarios
tend to happen relatively infrequently:
...
digraph G {
  ...
  zip [shape=record label="<f0> Foo.class | <f1> MANIFEST.MF | <f2> central directory"]
...
`central directory` with updated pointers:
...
digraph G {
  label="archive.zip"
  node [shape=box width=0 height=0 style=filled fillcolor=white]
  zip [shape=record label="<f0> Foo.class | <f1> MANIFEST.MF | <f3> additional files... | <f2> updated central directory"]
  zip:f2:n -> zip:f1:n [label="reverse offsets"]
  zip:f2:n -> zip:f0:n
  zip:f2:n -> zip:f3:n
}
...
When adding files to an existing archive, the existing files do not need to be touched at all,
...
1. Makes a copy of the upstream assembly. Copy a file is typically fast even
...
from ~20s to ~1s v.s. the equivalent workflow in other build tools like Maven or Gradle.
