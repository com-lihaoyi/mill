= How To Easily Publish to Maven Central with Mill
// tag::header[]
:author: Li Haoyi
:revdate: 3 Feb 2026
...
set up multiple build tool plugins, and so on. This article explores the JVM build tool greatly
simplifies the process of publishing to Maven Central, so that anyone can easily package and upload
their code to Maven Central to share it with the rest of the JVM community.
...
The workflow for this section is the same for Mill as it is for any other build too, as
fundamentally you need to prove to Sonatype (who runs the repository) that you own the
domain name or other namespace, to prevent others from potentially impersonating you. You
need to do this once for every organization/domain you own, e.g. verifying I own the domain
`lihaoyi.com` so I can publish under the organization `com.lihaoyi`, but this typically only
...
This script is committed to your codebase, so anyone who checks out the code will have the
`./mill` script available and ready to use.

== Configuring your Build File

The next step is to configure your `build.mill.yaml` file with the
xref:mill::javalib/publishing.adoc#_basic_publishing_configuration[necessary Java project metadata]
that Maven Central expects: the `publishVersion`, `artifactName`, and some other miscellaneous
settings that need to be included in the published `pom.xml` file:
...
Mill automatically generates the necessary `pom.xml` for you when publishing, so you
just need to provide the key-value pairs above and the tool will take care of the rest.

Mill supports all 3 major JVM languages, so if you want to xref:mill::scalalib/publishing.adoc#_basic_publishing_configuration[publish Scala code], you can use
`ScalaModule` rather than `JavaModule`:
...
Or if you want to xref:mill::kotlinlib/publishing.adoc#_basic_publishing_configuration[publish Kotlin code], `KotlinModule`:
...
set up, Mill's `mill.javalib.SonatypeCentralPublishModule/initGpgKeys`
...
This provides the generated PGP secret for you as BASE64 formatted strings
...
You can verify that your library is available online by copying your `./mill` bootstrap script
into an empty folder and opening a
...
--------------------------------------------
  com.lihaoyi:example:0.0.1
	...
	not found: https://repo1.maven.org/maven2/com/lihaoyi/example/0.0.1/example-0.0.4.pom
--------------------------------------------
```
...
=== Automatic Tag Versioning

Lastly, it is common to want to automatically assign the publish version based on the most recent
git tag applied to the repository. To this, we can replace the hardcoded `publishVersion` above
with the mixin `mill.util.VcsVersionModule` below, which wires up `publishVersion` to infer the
