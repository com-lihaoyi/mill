...
set up multiple build tool plugins, and so on. This article explores how the Mill build tool makes
it easy to publish to Maven Central, so that anyone can easily package and upload
their code to share it with the rest of the JVM community.
...
need to do this for every organization/domain you own, e.g. verifying I own the domain
`lihaoyi.com` so I can publish under the organization `com.lihaoyi`, but for most people
...
`./mill` script available and ready to use. Notably, you don't need to `brew`/`apt`/`winget`/etc.
install Mill, nor do you need to `brew`/`apt`/`winget` install Java or https://sdkman.io/[SdkMan],
as the `./mill` script (`mill.bat` on windows) automatically downloads and caches them as necessary.
...
is tedious to keep doing. Mill has the ability to  automatically assign the publish version based
on the most recent git tag applied to the repository, to save you that trouble.

To this, we can remove the hardcoded `publishVersion` above by extending the class
`mill.util.VcsVersionModule` below, which wires up `publishVersion` to infer the
