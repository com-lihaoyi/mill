...
as SBT defaults to limiting the parallelism only for forked tests.
Here we can see that the _SBT Serial_ benchmark completes  about as fast as Mill's
_Serial Execution_, while _SBT Parallel_ runs about as fast as Mill's _Biased Dynamic
Sharding_. Although the Mill's _Biased Dynamic Sharding_ isn't any faster than the
SBT equivalent, it is able to provide the additional flexibility and isolation benefits
of running tests in a subprocess without sacrificing any performance.
