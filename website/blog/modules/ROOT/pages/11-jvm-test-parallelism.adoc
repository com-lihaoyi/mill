...
Mill, it also outperforms running tests using other build tools like Maven or Gradle,
even when it is running the exact same tests on the exact same codebase. Unlike previous
attempts which would "fall off a cliff" on pathological workloads, _biased dynamic sharding_
seems to provide good performance on all the workloads that we benchmarked.
