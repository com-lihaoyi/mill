...
runner compare to other JVM build tools like Maven, Gradle, or SBT?
...
strategies and Maven to see how they compare. To run the same subset of unit test suites using Maven that we
...
For another data point, we repeated the benchmarks we did on the Netty on the
https://github.com/mockito/mockito[Mockito] codebase. Mockito is a popular mocking framework
for JVM unit tests, and is commonly in Java, Kotlin, and Scala projects, and its
...
not 100% complete can serve to let us compare the Mill test parallelism strategy to that
...
|===
| | *Gradle Serial* | *Gradle Parallel* | *Gradle Parallel + maxParallelForks*
| Mockito unit tests |  *90s* | *56s* | *31s*
|===

Gradle defaults to parallelizing tests at a subproject level similar to Mill's _Module Sharding_,
and so the performance is similar. Enabling
...
|===
| | *SBT test*
| Gatling unit tests |  *25s*
|===

For reasons I haven't understood, even though most SBT build tasks run in parallel,
SBT seems to run tests following a _Serial Execution_ strategy, and the time taken
matches almost exactly how long Mill takes to run the tests using the same strategy.
But from there, we can see that _Module Sharding_, _Dynamic Sharding_, and
_Biased Dynamic Sharding_ all provide significant speedups, with _Biased Dynamic Sharding_
running the same tests in less than half the time it takes SBT or Mill to run them serially.
