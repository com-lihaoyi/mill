...
You could also run the program using the Mill build tool instead of Maven. But while
...
Mill 1.1.0 tries to change that.
...
Unlike a Maven `pom.xml`, Mill's `build.mill.yaml` files contain only the configuration you need,
and nothing else. Compared to the equivalent Maven `pom.xml`, `build.gradle`, or programmable
`build.mill` files, Mill's `build.mill.yaml` files are really lightweight, without Maven's reams
of XML boilerplate or Gradle's complex Groovy/Kotlin syntax. This makes them easy
...
production machine it may be running on. Anyone invoking the `./mill` bootstrap script
automatically installs and caches the correct versions of Mill, `java`, `javac`, etc. to
work with:
...
configuration overrides. For example, here
...
* Declarative `build.mill.yaml` files can inter-operate with programmable `build.mill` files
  via xref:mill::javalib/module-config.adoc#_custom_module_traits[custom module traits] and
  xref:mill::javalib/module-config.adoc#_programmable_moduledeps[depending on programmable modules],
  so you can mix-and-match the two config styles within a large project depending on which is more
  appropriate for each module.

* Setting `mvnDeps`, `repositories`, `mainClass`, `jvmVersion`, etc. means the same thing in both
  styles. Although the syntax is different, the semantics are the same:
  both declarative and programmable config styles used the same config keys with the same meaning,
  documented by the same {mill-doc-url}/api/latest/mill.html[Mill API Reference].
