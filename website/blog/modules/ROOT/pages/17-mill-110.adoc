...
scripts. The newly-released Mill build tool v1.1.0 contains two major features
that try to improve upon this pain point:
xref:#_declarative_mill_builds[declarative build.mill.yaml configuration files], and support for
xref:#_mill_single_file_scripts[single-file Java, Scala, and Kotlin programs]
with their configuration in a build header comment. This allows your project's build to be
configured via a compact `build.mill.yaml` file:
...
they will streamline the process of getting started with Java programs, and opening up new
use cases for writing small-scale programs and scripts in Java and other JVM languages.
...
you do have custom build logic that needs programming, it comes at a cost:

1. Programmable configuration is inherently more complicated than declarative configuration

2. Not every project requires custom build logic, and so the added complexity is
   pure downside without benefit.

For simple projects with simple builds, programmable configuration adds a lot of
complexity for no gain. So for simple Java projects the only suitable build tool was
https://maven.apache.org/[Maven]. Maven has its strength and weaknesses, but for simple
Java programs you really had no other option: simple projects work best with declarative
configuration, and Maven is the only declarative build tool in the JVM ecosystem.
...
and the necessary config is in the xref:#_foot_notes[]. I have no doubt most people would be
able to get __something__ working on their machine. But for someone trying to write a small
program that can build and run reliably on their laptop, their colleagues' laptops, their CI
machines and their production environment, these things can end up being surprisingly tricky.

You could run the program using the Mill build tool instead of Maven, which has some benefits.
Mill's xref:mill::cli/installation-ide.adoc#_bootstrap_scripts[Bootstrap Scripts] manage
xref:16-zero-setup.adoc[both your Mill and your JVM installation] for you so you don't need
to install things manually, and Mill's command-line invocation syntax `./mill run <arg1> <arg2>`
is a lot more intuitive than Maven's. But while Mill `build.mill` files are a lot more concise
than Maven ``pom.xml``s, the amount of irrelevant things you have to do due to their programmable
nature makes them still pretty involved:
...
The first step Mill takes towards solving the friction of running small JVM programs is
...
files are really lightweight and both easy to write and easy to read.
...
as appropriate, letting you pick whatever makes sense at the moment knowing that
you can easily transition to the other style later, in part or in full.
...
1. With Mill's xref:mill::cli/installation-ide.adoc#_bootstrap_scripts[Bootstrap Scripts],
   ...
   codebase is guaranteed to get the correct versions configured for that project when
   running `./mill`
...
so scripts and other small programs tended to get written in other languages like Python, Bash,
or Javascript. But with Mill's new single-file script support, we hope that running a Java,
Scala, or Kotlin script from the command-line is as easy as running a script or executable
written in any other language, and "scripting using the JVM" can finally have its time to shine.
