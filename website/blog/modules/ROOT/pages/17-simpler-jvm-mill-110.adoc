...
(https://github.com/com-lihaoyi/mill/blob/main/changelog.adoc#110[changelog])
...
{mill-doc-url}/api/latest/mill/javalib/JavaModule.html#mvnDeps-0[full range of configuration
keys and tasks]: `jvmVersion`, `javacOptions`, `mvnDeps`, `jshell`, etc.
...
4. Mill scripts can xref:mill::javalib/script.adoc#_project_moduledeps[depend on a larger project's modules]
or xref:mill::javalib/script.adoc#_custom_script_module_classes[share their configuration]

5. Mill scripts have xref:mill::javalib/script.adoc#_script_ide_integration[full support in IDEs]
like IntelliJ or VSCode (or forks like Cursor or Windsurf)

_Mill Script IntelliJ Support_

image::ScriptIntellijSupport.png[]

_Mill Script VSCode Support_

image::ScriptVSCodeSupport.png[]

Using Java, Scala or Kotlin to write small scripts isn't a new idea. The ability to run
`java Foo.java` from the command line was introduced in Java 11, and tools like JBang providing
...
Like the rest of Mill, single-file scripts support the three major JVM languages. For example,
if you think the `HtmlScraper.java` is too verbose, you may want to
xref:mill::kotlinlib/script.adoc#_html_web_scraper[write your HtmlScraper in Kotlin] instead:
...
Or you may want to write your
xref:mill::scalalib/script.adoc#_html_web_scraper[HtmlScraper in Scala], which feels
similar to scripting in Ruby, Python, or Javascript, but with the performance,
parallelizability and type-safety of a JVM language:
...
Java is not the only JVM language, and languages like Kotlin or Scala complement it
...
from the robust runtime and rich ecosystem of the JVM.
...
they are often used for large enterprise codebases.
