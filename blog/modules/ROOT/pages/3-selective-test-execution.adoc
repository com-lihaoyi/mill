...
# Selective Test Execution in Mill 0.12.4
...
These various modules would typically each have their own test suite: e.g. each backend
service would have `backend_service_1_tests`, `backend_service_2_tests`, frontend
web codebases `frontend_web_1_tests`, `frontend_web_2_tests`, utility modules
`backend_service_utils_tests`, `frontend_web_utils_tests`, etc. Even the deployments
...
digraph G {
  ...
  deployment_1 -> staging_environment [style=dashed]
  deployment_2 -> staging_environment [style=dashed]
  deployment_3 -> staging_environment [style=dashed]
  staging_environment [style=dashed]
  end_to_end_tests_1 [color=red, penwidth=2]
  end_to_end_tests_2
  end_to_end_tests_3

  staging_environment -> end_to_end_tests_1 [style=dashed]
  staging_environment -> end_to_end_tests_2 [style=dashed]
  staging_environment -> end_to_end_tests_3 [style=dashed]

  deployment_1 -> end_to_end_tests_1  [color=red, penwidth=2]
  deployment_2 -> end_to_end_tests_2
  deployment_3 -> end_to_end_tests_3
}
```
...
xref:mill::large/selective-execution.adoc[Selective Execution] supports for
<<Dependency-based Test Selection>> out of the box.