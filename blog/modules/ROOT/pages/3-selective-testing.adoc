...
level_, so even custom tasks and overrides can benefit from it.

Although Mill provides better support for selective testing that most build tools (which provide
none), the <<Limitations of Dependency-based Selective Testing>> do cause issues. Even in Mill's
own pull-request validation jobs, the fact that the most expensive integration or end-to-end
tests are selected every time causes slowness. What kind of <<Heuristic-based Selective Testing>>
can help improve things is an open question.
...
https://mill-build.org/[Mill Build Tool] a try!
