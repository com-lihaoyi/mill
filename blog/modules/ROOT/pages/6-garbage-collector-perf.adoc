...
poorly understood. In developing the https://mill-build.org[Mill JVM build tool],
...
To understand how the real-world JVM garbage collector works, it is best to start
...
heap, we will need to do a collection:
...
digraph G {
  ...
  heap1:f5:s -> alloc:n [dir=back, style=dotted]
  alloc [label = "free memory", shape=plaintext]GC 
  stack [shape=record label="STACK | <f0> | <f1> | <f2> | <f3> | <f4> | <f5> | <f6> | <f7> "]
...
This collection has free up 5 slots, so we now have space to allocate the
...
digraph G {
  ...
  stack:f3 -> heap2:f4 [dir=none, style=dashed]
  stack:f4 -> heap2:f5 [dir=none, style=dashed]
...
digraph G {
  ...
  stack:f3 -> heap1:f3 [dir=none, style=dashed, color=red, penwidth=3]
  stack:f4 -> heap1:f4 [dir=none, style=dashed, color=red, penwidth=3]
  stack:f5 -> heap1:f5 [dir=none, style=dashed, color=green, penwidth=3]
  stack:f6 -> heap1:f6 [dir=none, style=dashed, color=green, penwidth=3]
...
There are a few major advantages that tracing garbage collector has over
https://en.wikipedia.org/wiki/Reference_counting[Reference Counting], another popular
...
digraph G {
  ...
  node [shape=box width=0 height=0 style=filled fillcolor=white]
  heap1 [shape=record label="HEAP | <f0> new1 | <f1> new2 | <f2> new3 | <f3> new4 | <f4> new5 | <f5> new6 | <f6> new7 | <f7> | <f8> | <f9> | <f10> | <f11> | <f12> | <f13> | <f14> | <f15>"]
...
digraph G {
  ...
  stack [shape=record label="STACK | <f0> | <f1> | <f2> | <f3> | <f4> | <f5> | <f6> | <f7> "]
  stack:f0 -> heap:f0 [dir=none, style=dashed]
  stack:f1 -> heap:f1 [dir=none, style=dashed]
  stack:f2 -> heap:f4 [dir=none, style=dashed]
  stack:f3 -> heap:f5 [dir=none, style=dashed]
...
and software you run it (I ran it on a M1 Macbook Pro running Java 17). But the results should be
...
I used the following https://ammonite.io/[Ammonite Scala script] to run the benchmark for a
range of inputs to collect the numbers shown below:

- xref:attachment$GC-bench.sc[GC-bench.sc]
...
2. *GC pause times are relatively constant regardless of the heap size*.
   ...
   `6400 mb` heap (8 times as large!) has a `93 ms` pause time. There's some variation
   not a lot
...
that cannot afford to just randomly stop for 100ms at a time.
...
1. *Caching data _in-process_ can make garbage collection pause times _worse_!* If your
...
2. *In contrast, caching things _out of process_ does not have this problem.* So feel free
   ...
   These are not traced by your program's GC, and will not slow it down.
...
3. *Collecting Garbage doesn't actually slow the garbage collector down!* Most of the GC time
   ...
   the time taken to run the collection will be the same regardless
...
5. *Even when to declare the memory "enough" or "not enough" can be very arbitrary.*
...
7. *Fewer larger processes can have worse GC performance than more smaller processes, even if
   they do the same things!* There are
