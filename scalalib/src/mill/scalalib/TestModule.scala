...
object TestModule {
  ...
  trait Junit4 extends TestModule {
    ...
    override def mandatoryLibraryDeps: T[Seq[Dep]] = Task {
      super.mandatoryLibraryDeps() ++ Seq(ivy"${mill.scalalib.api.Versions.sbtTestInterface}")
    }
  }
  ...
  trait Junit5 extends TestModule {
    ...
    override def mandatoryLibraryDeps: T[Seq[Dep]] = Task {
      super.mandatoryLibraryDeps() ++ Seq(ivy"${mill.scalalib.api.Versions.jupiterInterface}")
    }
     ...
     * `sbt-jupiter-interface` dependency from `libraryDeps`, make sure to also
  ...
  trait JavaModuleBase extends BspModule {
    def libraryDeps: T[Seq[Dep]] = Seq.empty[Dep]
    def mandatoryLibraryDeps: T[Seq[Dep]] = Seq.empty[Dep]
