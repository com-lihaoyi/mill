...
object TestModule {
  ...
  trait TestNg extends TestModule {
    ...
    override def mandatoryJvmDeps: T[Seq[Dep]] = Task {
      super.mandatoryJvmDeps() ++ Seq(
        jvm"com.lihaoyi:mill-contrib-testng:${mill.api.BuildInfo.millVersion}"
      )
    }
  }
  ...
  trait Junit4 extends TestModule {
    ...
    override def mandatoryJvmDeps: T[Seq[Dep]] = Task {
      super.mandatoryJvmDeps() ++ Seq(jvm"${mill.scalalib.api.Versions.sbtTestInterface}")
    }
  }
  ...
  trait Junit5 extends TestModule {
    ...
    override def mandatoryJvmDeps: T[Seq[Dep]] = Task {
      super.mandatoryJvmDeps() ++ Seq(jvm"${mill.scalalib.api.Versions.jupiterInterface}")
    }
     ...
     * `sbt-jupiter-interface` dependency from `jvmDeps`, make sure to also
  ...
  trait JavaModuleBase extends BspModule {
    def jvmDeps: T[Seq[Dep]] = Seq.empty[Dep]
    def mandatoryJvmDeps: T[Seq[Dep]] = Seq.empty[Dep]
