...
trait AndroidModule extends JavaModule {
  ...
  def androidUnpackArchives: T[Seq[UnpackedDep]] = Task {
    ...
    //
    // In Gradle terms using only `resolvedRunJvmDeps` won't be complete, because source modules can be also
    // api/implementation, but Mill has no such configurations.
    val aarFiles = (super.compileClasspath() ++ super.resolvedRunJvmDeps())
      .map(_.path)
      .filter(_.ext == "aar")
      .distinct
