...
//  def androidIsDebug: T[Boolean] = true
  ...
  def androidDebugKeystore: Task[PathRef] = Task(persistent = true) {
...
//  }
  ...
  def androidReleaseLibsRClasses: T[Seq[PathRef]] = Task {
    // TODO do this better
    ...
    val rClassDir = androidReleaseResources()._1.path / rClassDirName
  ...
  def androidDebugLibsRClasses: T[Seq[PathRef]] = Task {
    // TODO do this better
    // So we have application R.java class generated by aapt2 link, which includes IDs of app resources + libs resources.
    // But we also need to have R.java classes for libraries. The process below is quite hacky and inefficient, because:
    // * it will generate R.java for the library even library has no resources declared
    // * R.java will have not only resource ID from this library, but from other libraries as well. They should be stripped.
    val rClassDir = androidDebugResources()._1.path / rClassDirName
    val mainRClassPath = os.walk(rClassDir)
      .find(_.last == "R.java")
      .get
   ...
   */
  def androidReleaseResources: T[(PathRef, Seq[PathRef])] = Task {
  ...
  def androidDebugResources: T[(PathRef, Seq[PathRef])] = Task {
    val rClassDir = T.dest / rClassDirName
    val compiledResDir = T.dest / compiledResourcesDirName
    os.makeDir(compiledResDir)
    val compiledResources = collection.mutable.Buffer[os.Path]()
...
//  }
  ...
  implicit val builderReadWriter
    ...
    readwriter[Seq[String]].bimap(
      // Convert builder to Seq when serializing
      builder => builder.result(),
      // Convert Seq to a builder when deserializing
      seq => {
   ...
   */
  def androidReleaseDex: T[PathRef] = Task {
...
//    os.write(proguardFile, knownProguardRules)
    ...
    d8ArgsBuilder ++= Seq(
      "--output",
      androidSources.androidReleaseDex.path.toString,
      "--main-dex-rules",
    ...
    PathRef(androidSources.androidReleaseDex.path)
  }

  def androidDebugDex: T[PathRef] = Task {
...
//    os.write(proguardFile, knownProguardRules)
    ...
    d8ArgsBuilder ++= Seq(
      "--output",
      androidSources.androidDebugDex.path.toString,
      "--main-dex-rules",
      proguardFile.toString()
    ) ++ appCompiledFiles ++ libsJarFiles
    ...
    PathRef(androidSources.androidDebugDex.path)
  }
   ...
   */
  def androidReleaseUnsignedApk: T[PathRef] = Task {
    val unsignedApk = Task.dest / "app.unsigned.apk"

    os.copy(androidReleaseResources()._1.path / "res.apk", unsignedApk)
    val dexFiles = os.walk(androidReleaseDex().path)
      .filter(_.ext == "dex")
  ...
  def androidDebugUnsignedApk: T[PathRef] = Task {
    val unsignedApk = Task.dest / "app.unsigned.apk"

    os.copy(androidDebugResources()._1.path / "res.apk", unsignedApk)
    val dexFiles = os.walk(androidDebugDex().path)
      .filter(_.ext == "dex")
      .map(os.zip.ZipSource.fromPath)
    // TODO probably need to merge all content, not only in META-INF of classes.jar, but also outside it
    val metaInf = androidLibsClassesJarMetaInf()
   ...
   */
  def androidReleaseSignKeyDetails: T[Seq[String]] = Task {
    ...
    Seq(
      ...
      androidMinSdk().toString,
      "--ks",
      androidReleaseKeyPath().get.path.toString,
      "--ks-key-alias",
      keyAlias,
      "--ks-pass",
      s"pass:$keystorePass",
  ...
  def androidDebugSignKeyDetails: T[Seq[String]] = Task {
    ...
    Seq(
      ...
      androidMinSdk().toString,
      "--ks",
      androidDebugKeystore().path.toString,
      "--ks-key-alias",
      keyAlias,
      "--ks-pass",
      s"pass:$keystorePass",
   ...
   */
  def androidReleaseAlignedUnsignedApk: T[PathRef] = Task {
    ...
    os.call((
      ...
      "-p",
      "4",
      androidReleaseUnsignedApk().path,
      alignedApk
    ))
  ...
  def androidDebugAlignedUnsignedApk: T[PathRef] = Task {
    val alignedApk: os.Path = Task.dest / "app.aligned.apk"
   ...
   */
  def androidReleaseApk: T[PathRef] = Task {
    ...
    val signArgs = Seq(
      ...
      "sign",
      "--in",
      androidReleaseAlignedUnsignedApk().path.toString,
      "--out",
      signedApk.toString
    ) ++ androidReleaseSignKeyDetails()
  ...
  def androidDebugApk: T[PathRef] = Task {
    val signedApk = Task.dest / "app.apk"
    ...
    val signArgs = Seq(
      ...
      "sign",
      "--in",
      androidDebugAlignedUnsignedApk().path.toString,
      "--out",
      signedApk.toString
    ) ++ androidDebugSignKeyDetails()
  ...
  def mainReleaseDexRules: T[Option[PathRef]] = T {
    Some(PathRef(androidReleaseResources()._1.path / "main-dex-rules.pro"))
  }

  def mainDebugDexRules: T[Option[PathRef]] = T {
    Some(PathRef(androidDebugResources()._1.path / "main-dex-rules.pro"))
  }
