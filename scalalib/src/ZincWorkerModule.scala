...
import mill.Agg
import mill.T
import mill.api.{Ctx, FixSizedCache, KeyedLockedCache, Loose}
...
trait ZincWorkerModule extends mill.Module {
  ...
  def backgroundWrapperClasspath =
    T {
      mill.modules.Util.millProjectModule(
  ...
  def worker: Worker[mill.scalalib.api.ZincWorkerApi] =
    T.worker {
      val ctx = T.ctx()
