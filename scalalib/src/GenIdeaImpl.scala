...
case class GenIdeaImpl(evaluator: Evaluator, ctx: Log with Home, rootModule: BaseModule, discover: Discover[_]) {
  ...
  def xmlFileLayout(
    ...
    val buildLibraryPaths: immutable.Seq[Path] =
      if (!fetchMillModules) Nil
        ...
        Option(mill.modules.Util.millProperty("MILL_BUILD_LIBRARIES")) match {
          case Some(found) => found.split(',').map(os.Path(_)).distinct.toList
          case None =>
            val repos = modules.foldLeft(Set.empty[Repository])(_ ++ _._2.repositories) ++ Set(
      ...
      confs.foreach { conf =>
        ...
        seen.get(key) match {
          ...
          case Some(existing) =>
            ...
            val msg =
              s"Config collision in file `${conf.name}` and component `${conf.component}`: ${details(conf.config)} vs. ${details(existing)}"
    ...
    val compilerSettings = resolved
      .foldLeft(Map[(Loose.Agg[os.Path], Seq[String]), Vector[JavaModule]]()) { (r, q) =>
    ...
    val moduleFiles = resolved.map {
      case ResolvedModule(path, resolvedDeps, mod, _, _, _, _, facets, _, compilerOutput) =>
        val Seq(
...
//    simpleIdeaConfigFileTemplate(Map("ScalaProjectSettings" -> Map("scFileMode" -> "Ammonite")))
  ...
  def moduleXmlTemplate(
    ...
    <module type="JAVA_MODULE" version={"" + ideaConfigVersion}>
      ...
      } yield <content url={"file://$MODULE_DIR$/" + path}>
