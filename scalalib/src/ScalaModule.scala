...
trait ScalaModule extends JavaModule { outer =>
  ...
  override def mapDependencies =
    T.task { d: coursier.Dependency =>
      val artifacts =
        if (isDotty(scalaVersion()))
  ...
  /**
    ...
    */
  def scalaCompilerClasspath: T[Agg[PathRef]] =
    T {
      resolveDeps(
        T.task {
  ...
  override def compile: T[mill.scalalib.api.CompilationResult] =
    T.persistent {
