...
import mill.api.{BuildProblemReporter, PathRef}
...
trait ZincWorkerApi {
  ...
  def discoverMainClasses(compilationResult: CompilationResult)(implicit ctx: ZincWorkerApi.Ctx): Seq[String]
...
object Util {
  ...
  def grepJar(classPath: Agg[os.Path], name: String, versionPrefix: String, sources: Boolean = false) = {
    ...
    classPath.iterator
      ...
      .getOrElse(
        throw new Exception(
          s"Cannot find **/$name-$versionPrefix*$suffix or **/$versionPrefix*/$dir/$name$suffix in ${classPath.iterator
            .mkString("[", ", ", "]")}"
        )
      )
  }
  ...
  def scalaNativeBinaryVersion(version: String) =
    version match {
  ...
  def scalaJSNativeWorkerVersion(version: String) =
    version match {
   ...
   */
  def scalaJSUsesTestBridge(scalaJSVersion: String): Boolean =
    scalaJSVersion match {
