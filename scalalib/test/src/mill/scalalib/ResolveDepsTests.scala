...
object ResolveDepsTests extends TestSuite {
  ...
  object TestCase extends TestBaseModule {
    object pomStuff extends JavaModule {
      def jvmDeps = Seq(
        ...
        jvm"org.apache.hadoop:hadoop-yarn-server:3.4.0"
      )
    ...
    object scope extends JavaModule {
      def jvmDeps = Seq(
        jvm"androidx.compose.animation:animation-core:1.1.1",
        jvm"androidx.compose.ui:ui:1.1.1"
      )
  ...
  val tests = Tests {
    test("resolveValidDeps") {
      val deps = Seq(jvm"com.lihaoyi::pprint:0.5.3")
      val Success(paths) = evalDeps(deps): @unchecked
      assert(paths.nonEmpty)
    }

    test("resolveValidDepsWithClassifier") {
      val deps = Seq(jvm"org.lwjgl:lwjgl:3.1.1;classifier=natives-macos")
    ...
    test("resolveTransitiveRuntimeDeps") {
      val deps = Seq(jvm"org.mockito:mockito-core:2.7.22")
      assertRoundTrip(deps, simplified = true)
      val Success(paths) = evalDeps(deps): @unchecked
      assert(paths.nonEmpty)
    ...
    test("excludeTransitiveDeps") {
      val deps = Seq(jvm"com.lihaoyi::pprint:0.5.3".exclude("com.lihaoyi" -> "fansi_2.12"))
      assertRoundTrip(deps, simplified = true)
      val Success(paths) = evalDeps(deps): @unchecked
    ...
    test("excludeTransitiveDepsByOrg") {
      val deps = Seq(jvm"com.lihaoyi::pprint:0.5.3".excludeOrg("com.lihaoyi"))
      assertRoundTrip(deps, simplified = true)
      val Success(paths) = evalDeps(deps): @unchecked
    ...
    test("excludeTransitiveDepsByName") {
      val deps = Seq(jvm"com.lihaoyi::pprint:0.5.3".excludeName("fansi_2.12"))
      assertRoundTrip(deps, simplified = true)
      val Success(paths) = evalDeps(deps): @unchecked
      assert(!paths.exists(_.path.toString.contains("fansi_2.12")))
    }
    ...
    test("errOnInvalidOrgDeps") {
      val deps = Seq(jvm"xxx.yyy.invalid::pprint:0.5.3")
      assertRoundTrip(deps, simplified = true)
    ...
    test("errOnInvalidVersionDeps") {
      val deps = Seq(jvm"com.lihaoyi::pprint:invalid.version.num")
      assertRoundTrip(deps, simplified = true)
      val Failure(errMsg) = evalDeps(deps): @unchecked
    ...
    test("errOnPartialSuccess") {
      val deps = Seq(jvm"com.lihaoyi::pprint:0.5.3", jvm"fake::fake:fake")
      assertRoundTrip(deps, simplified = true)
      val Failure(errMsg) = evalDeps(deps): @unchecked
