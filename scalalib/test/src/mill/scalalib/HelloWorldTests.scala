  ...
  def tests: Tests = Tests {
    ...
    test("compile") {
      ...
      test("recompileOnChange") - UnitTester(HelloWorld, sourceRoot = resourcePath).scoped { eval =>
        ...
        os.write.append(HelloWorld.modulePath / "core/src/Main.scala", "\n")
      ...
      test("failOnError") - UnitTester(HelloWorld, sourceRoot = resourcePath).scoped { eval =>
        os.write.append(HelloWorld.modulePath / "core/src/Main.scala", "val x: ")
        ...
        // Works when fixed
        os.write.over(
          HelloWorld.modulePath / "core/src/Main.scala",
          os.read(HelloWorld.modulePath / "core/src/Main.scala").dropRight(
            "val x: ".length
          )
        )
