...
object HelloWorldTests extends TestSuite {
  ...
  val helloWorldWithMainEvaluator =
    new TestEvaluator(HelloWorldWithMain, outPath, workingSrcPath)
  val helloWorldWithMainAssemblyEvaluator =
    new TestEvaluator(HelloWorldWithMainAssembly, outPath, workingSrcPath)
  val helloWorldFatalEvaluator =
    new TestEvaluator(HelloWorldFatalWarnings, outPath, workingSrcPath)
  val helloWorldOverrideEvaluator =
    new TestEvaluator(HelloWorldScalaOverride, outPath, workingSrcPath)
  val helloWorldCrossEvaluator =
    new TestEvaluator(CrossHelloWorld, outPath, workingSrcPath)
  ...
  def tests: Tests = Tests {
    ...
    'scalaVersion - {
      'fromBuild - {
        ...
        assert(result == "2.12.4", evalCount > 0)
      }
      'override - {
        val Right((result, evalCount)) =
          helloWorldOverrideEvaluator(HelloWorldScalaOverride.scalaVersion)

        assert(result == "2.11.11", evalCount > 0)
      }
    }
    'scalacOptions - {
      'emptyByDefault - {
        val Right((result, evalCount)) =
          helloWorldEvaluator(HelloWorld.scalacOptions)

        assert(result.isEmpty, evalCount > 0)
      }
      'override - {
        val Right((result, evalCount)) =
    ...
    'compile - {
      'fromScratch - {
        ...
        val expectedClassfiles =
          compileClassfiles.map(outPath / 'compile / 'dest / 'classes / _)
        assert(
      ...
      'failOnError - {
        ...
        val paths =
          Evaluator.resolveDestPaths(outPath, HelloWorld.compile.ctx.segments)

        assert(ls.rec(paths.dest / 'classes).isEmpty, !exists(paths.meta))
        // Works when fixed
      ...
      'passScalacOptions - {
        ...
        val Left(Result.Exception(err, _)) =
          helloWorldFatalEvaluator(HelloWorldFatalWarnings.compile)
    ...
    'runMain - {
      ...
      'notRunInvalidMainObject - {
        val Left(Result.Exception(err, _)) =
          helloWorldEvaluator(HelloWorld.runMain("Invalid"))

        assert(err.isInstanceOf[InteractiveShelloutException])
      }
      'notRunWhenComplileFailed - {
        write.append(mainObject, "val x: ")

        val Left(Result.Exception(err, _)) =
          helloWorldEvaluator(HelloWorld.runMain("Main"))
    ...
    'forkRun - {
      ...
      'notRunWithoutMainClass - {
        val Left(Result.Exception(err, _)) =
          helloWorldEvaluator(HelloWorld.forkRun())
    ...
    'run - {
      ...
      'notRunWithoutMainClass - {
        val Left(Result.Exception(err, _)) =
          helloWorldEvaluator(HelloWorld.run())
    ...
    'jar - {
      'nonEmpty - {
        val Right((result, evalCount)) =
          helloWorldWithMainEvaluator(HelloWorldWithMain.jar)
