...
object ScalaVersionsRangesTests extends TestSuite {
  object ScalaVersionsRanges extends TestUtil.BaseModule {
    ...
    class CoreCrossModule(val crossScalaVersion: String)
      ...
      override def sources = T.sources {
        super.sources() ++ Util
          .versionSpecificSources(crossScalaVersion, crossScalaVersions)
          .map(s => PathRef(millSourcePath / s"src-$s"))
      }
    }
  }
  val resourcePath =
  ...
  val tests = Tests {
    test("Scala 2.12- and 2.13+ specific code") {
