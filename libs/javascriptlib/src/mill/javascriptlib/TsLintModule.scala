...
trait TsLintModule extends Module {
  ...
  private def npmInstallLint: T[PathRef] = Task {
    Try(os.copy.over(mill.define.BuildCtx.workspaceRoot / ".npmrc", Task.dest / ".npmrc")).getOrElse(())
    os.call((
      "npm",
      "install",
      "--prefix",
  ...
  private def fmtConfig: T[Seq[PathRef]] = Task.Sources(
    mill.define.BuildCtx.workspaceRoot / "eslint.config.mjs",
    mill.define.BuildCtx.workspaceRoot / "eslint.config.cjs",
    mill.define.BuildCtx.workspaceRoot / "eslint.config.js",
    mill.define.BuildCtx.workspaceRoot / ".eslintrc.js",
    mill.define.BuildCtx.workspaceRoot / ".eslintrc.mjs",
    mill.define.BuildCtx.workspaceRoot / ".eslintrc.cjs",
    mill.define.BuildCtx.workspaceRoot / ".prettierrc",
    mill.define.BuildCtx.workspaceRoot / ".prettierrc.json"
  )
  ...
  def checkFormatEslint(args: mill.define.Args): Command[Unit] = Task.Command {
    resolvedFmtConfig() match {
      case Eslint =>
        val cwd = mill.define.BuildCtx.workspaceRoot
