...
trait PmdModule extends JavaModule {
  ...
  def pmd(@mainargs.arg pmdArgs: PmdArgs): Command[Int] = Task.Command {
    val (output, exitCode) = pmd0(pmdArgs.stdout, pmdArgs.sources)()
    pmdHandleErrors(pmdArgs.stdout, pmdArgs.check, exitCode, output)
  }

  protected def pmd0(stdout: Boolean, leftover: mainargs.Leftover[String]) = Task.Anon {
    val output = pmdOutput().path
    val args = pmdOptions() ++
      Seq(
        "-d",
  ...
  /** PMD output report. */
  def pmdOutput: T[PathRef] = Task {
    PathRef(Task.dest / s"pmd-output.${pmdFormat()}")
  }
