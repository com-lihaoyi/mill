...
public abstract class ServerLauncher {
      ...
      if (locks.daemonLock.probe()) {
        ...
        if (maybeDaemonProcess.isPresent()) {
          ...
          var maybeLaunchFailed =
              waitUntilDaemonTakesTheLock(locks.daemonLock, daemonDir, daemonProcess);
          if (maybeLaunchFailed.isPresent()) {
            ...
            log.accept(
                "The server " + daemonProcess + " has taken the daemon lock: " + locks.daemonLock);
  ...
  private static Optional<ServerLaunchOutputs> waitUntilDaemonTakesTheLock(
      Lock daemonLock, Path daemonDir, Process daemonProcess) throws Exception {
  ...
  private static Optional<ServerLaunchOutputs> checkIfLaunchFailed(
      Path daemonDir, Process daemonProcess) throws IOException {
