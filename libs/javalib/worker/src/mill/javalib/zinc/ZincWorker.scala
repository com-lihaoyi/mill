...
class ZincWorker(jobs: Int, useFileLocks: Boolean = false) extends AutoCloseable { self =>
  ...
  def identifyImpactedTests(
      op: ZincOp.IdentifyImpactedTests,
      processConfig: ZincWorker.ProcessConfig
  ): Result[(Seq[String], Map[String, String])] = {
    val _ = processConfig

    def getAnalysis(f: os.Path): Option[sbt.internal.inc.Analysis] = {
      if (os.exists(f)) {
        val opt = fileAnalysisStore(f).get()
        if (opt.isPresent) Some(opt.get().getAnalysis.asInstanceOf[sbt.internal.inc.Analysis])
        else None
      } else None
    }

    val currentAnalysisOpt = getAnalysis(op.compileOutput.analysisFile)
    if (currentAnalysisOpt.isEmpty) return Result.Success((Seq.empty, Map.empty))
    val currentAnalysis = currentAnalysisOpt.get

    val upstreamAnalyses = op.upstreamCompileOutput.flatMap(c => getAnalysis(c.analysisFile))
    val allAnalyses = upstreamAnalyses :+ currentAnalysis

    val currentStamps = mutable.Map.empty[String, String]
    val changedFiles = mutable.Set.empty[xsbti.VirtualFileRef]

    // Collect all current stamps and identify changed files
    allAnalyses.foreach { analysis =>
      analysis.stamps.getAllSourceStamps.forEach { (vf, stamp) =>
        val path = vf.id()
        val currentStamp = stamp.toString
        currentStamps.put(path, currentStamp)
        val prevStamp = op.previousStamps.get(path)
        if (!prevStamp.contains(currentStamp)) {
          changedFiles.add(vf)
        }
      }
    }

    // Also check for deleted files (files in previous stamps but not in current)
    val hasDeletedFiles = op.previousStamps.keys.exists(path => !currentStamps.contains(path))

    if (hasDeletedFiles) {
      // Since we can't get VirtualFileRef for deleted files, 
      // we'll need to handle this by running all tests
      Result.Success((Seq.empty, currentStamps.toMap))
    } else {
      // Find classes generated by changed files (these are directly impacted)
      val initialImpactedClasses = mutable.Set.empty[String]
      for (analysis <- allAnalyses) {
        val relations = analysis.relations
        for (src <- changedFiles) {
          val classesFromSource = relations.classes.forward(src)
          initialImpactedClasses ++= classesFromSource
        }
      }

      // Now find all transitive dependents using ONLY the test module's analysis
      // because we only care about tests that changed.
      val testRelations = currentAnalysis.relations
      val visited = mutable.Set.empty[String]
      val result = mutable.Set.empty[String]
      val queue = mutable.Queue.empty[String]
      queue.enqueueAll(initialImpactedClasses)

      while (queue.nonEmpty) {
        val cls = queue.dequeue()
        if (!visited(cls)) {
          visited.add(cls)
          
          // If this class is produced by a source file in the current test module, it's an impacted test.
          // We check this by seeing if there is any source file that maps to this class in the test relations.
          val sources = testRelations.classes.reverse(cls)
          if (sources.nonEmpty) {
            result.add(cls)
          }

          // Find all internal classes in the test module that depend on this class
          testRelations.internalClassDep.reverse(cls).foreach { dependent =>
            queue.enqueue(dependent)
          }
          testRelations.externalClassDep.reverse(cls).foreach { dependent =>
            queue.enqueue(dependent)
          }
        }
      }

      Result.Success((result.toSeq.sorted, currentStamps.toMap))
    }
  }
  ...
  def apply(
    ...
    op match {
      ...
      case msg: ZincOp.IdentifyImpactedTests =>
        identifyImpactedTests(msg, processConfig).asInstanceOf[op.Response]
    }
  }
}
