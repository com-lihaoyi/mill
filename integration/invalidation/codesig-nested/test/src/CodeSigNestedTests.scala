...
object CodeSigNestedTests extends IntegrationTestSuite {
  val tests: Tests = Tests {
    ...
    "nested" - {
      ...
      modifyFile(wsRoot / "build.sc", _.replace("; helperFoo }", "; helperFoo + 4 }"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("running qux", "running qux2"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace(" 1 ", " 6 "))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("running helperBar", "running helperBar2"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("20", "70"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("running helperQux", "running helperQux2"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("val valueBar = 0", "val valueBar = 10"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("val valueQux = 0", "val valueQux = 10"))
      ...
      modifyFile(wsRoot / "build.sc", _.replace("def qux", "\ndef qux"))
      val addedSingleNewline = eval("outer.inner.qux")
      assert(addedSingleNewline.out == "")

      modifyFile(wsRoot / "build.sc", _.replace("def", "\ndef"))
