...
object YamlHeaderKeyTests extends UtestIntegrationTestSuite {
  ...
  val tests: Tests = Tests {
    test - integrationTest { tester =>
      ...
      assert(res.err.contains("key \"mvnDep\" does not override any task, did you mean \"mvnDeps\"?"))

      assert(res.err.contains("mill-build/build.mill:3:14"))
      assert(res.err.contains("//| mill-jm-version: lols"))
      assert(res.err.contains("key \"mill-jm-version\" does not override any task, did you mean \"mill-jvm-version\"?"))
