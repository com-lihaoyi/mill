...
object BloopTests extends IntegrationTestSuite {
  ...
  val tests: Tests = Tests {
    test("test") - {
      ...
      test("root module bloop config should be created") - integrationTest { tester => import tester._
      ...
      test("mill-build module bloop config should be created") - integrationTest { tester => import tester._
        val installResult: Boolean = eval("mill.contrib.bloop.Bloop/install").isSuccess
      ...
      test("mill-build config should contain build.mill source") - integrationTest { tester => import tester._
        val millBuildJsonFile = workspacePath / ".bloop" / "mill-build-.json"
