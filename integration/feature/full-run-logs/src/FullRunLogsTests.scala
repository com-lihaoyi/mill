    ...
    test("noticker") - integrationTest { tester =>
      ...
      assert(
        res.err.replace('\\', '/').replaceAll("(\r\n)|\r", "\n") ==
          s"""|[info] compiling 4 Scala sources to ${tester.workspacePath}/out/mill-build/compile.dest/classes ...
              |[info] done compiling
      ...
      val expectedErrorRegex = java.util.regex.Pattern
        .quote(
          s"""|<dashes> run --text hello <dashes>
              ...
              |[build.mill-<digits>] [info] compiling 4 Scala sources to ${tester.workspacePath}/out/mill-build/compile.dest/classes ...
