...
object Deps {
  ...
  object Scalajs_0_6 {
    ...
    val scalajsSbtTestAdapter =
      ivy"org.scala-js::scalajs-sbt-test-adapter:0.6.33"
  ...
  object Scalajs_1 {
    ...
    val scalajsSbtTestAdapter =
      ivy"org.scala-js::scalajs-sbt-test-adapter:1.5.1"
  ...
  val ammonite = ivy"com.lihaoyi:::ammonite:2.3.8-67-4b6c67db"
  ...
  val windowsAnsi =
    ivy"io.github.alexarchambault.windows-ansi:windows-ansi:0.0.3"
...
trait MillPublishModule extends PublishModule {
  ...
  override def javacOptions =
    Seq("-source", "1.8", "-target", "1.8", "-encoding", "UTF-8")
}
...
trait MillModule extends MillApiModule { outer =>
  ...
  class Tests(ctx0: mill.define.Ctx)
      extends mill.Module()(ctx0)
      with super.Tests {
...
object scalajslib extends MillModule {
  ...
  def generatedBuildInfo = T {
    ...
    val content =
      s"""package ${packageNames.mkString(".")}
         ...
         |    val scalajsEnvNodejs = "${formatDep(
        Deps.Scalajs_1.scalajsEnvNodejs
      )}"
         |    val scalajsEnvJsdomNodejs = "${formatDep(
        Deps.Scalajs_1.scalajsEnvJsdomNodejs
      )}"
         |    val scalajsEnvPhantomJs = "${formatDep(
        Deps.Scalajs_1.scalajsEnvPhantomjs
         ...
         |""".stripMargin
    os.write(
      dir / packageNames / s"${className}.scala",
      content,
      createFolders = true
    )
    PathRef(dir)
  ...
  override def generatedSources: Target[Seq[PathRef]] = Seq(
    generatedBuildInfo()
  )
...
object contrib extends MillModule {
  ...
  object playlib extends MillModule {
    ...
    override def testArgs = T {
      val mapping = Map(
        "MILL_CONTRIB_PLAYLIB_ROUTECOMPILER_WORKER_2_6" -> worker("2.6")
          .assembly()
          .path,
        "MILL_CONTRIB_PLAYLIB_ROUTECOMPILER_WORKER_2_7" -> worker("2.7")
          .assembly()
          .path,
        "MILL_CONTRIB_PLAYLIB_ROUTECOMPILER_WORKER_2_8" -> worker("2.8")
          .assembly()
          .path
      )
  ...
  mill.modules.Jvm.universalScript(
    shellCommands = {
      ...
      def java(mainClass: String, passMillJvmOpts: Boolean) = {
        ...
        s"""exec "$$JAVACMD" $jvmArgsStr $$JAVA_OPTS $millJvmOpts -cp "${shellClassPath
          .mkString(":")}" $mainClass "$$@""""
      }
    ...
    cmdCommands = {
      ...
      def java(mainClass: String, passMillJvmOpts: Boolean) = {
        val millJvmOpts = if (passMillJvmOpts) "!mill_jvm_opts!" else ""
        s""""%JAVACMD%" $jvmArgsStr %JAVA_OPTS% $millJvmOpts -cp "${cmdClassPath
          .mkString(";")}" $mainClass %*"""
      }
...
object dev extends MillModule {
  ...
  def prependShellScript = T {
    val (millArgs, otherArgs) = forkArgs().partition(arg =>
    ...
    launcherScript(
      ...
      classpath,
      Agg(
        pathingJar().path.toString
      ) // TODO not working yet on Windows! see #791
    )
  }
  ...
  def pathingJar = T {
    ...
    mill.modules.Jvm
      .createJar(Agg(), mill.modules.Jvm.JarManifest(manifestEntries))
  }

  def run(args: String*) = T.command {
    args match {
      case Nil =>
        mill.eval.Result.Failure(
          "Need to pass in cwd as first argument to dev.run"
        )
      case wd0 +: rest =>
      ...
      os.walk(dir).foreach { file =>
        if (os.isFile(file) && file.ext == "html") {
          ...
          if (changed) {
            ...
            val newHtml = new SimpleHtmlSerializer(cleaner.getProperties())
              .getAsString(node)
...
def uploadToGithub(authKey: String) = T.command {
  ...
  if (label != millVersion())
    sys.error("Modified mill version detected, aborting upload")
