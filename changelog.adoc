...
Mill's `main` branch is targeting the upcoming Mill 1.0.0 release. This is a major backwards
incompatible release that aims to clean up years of technical debt, and build a strong
technical foundation for the next few years of development. We
expect it will take some work from projects on Mill 0.12.x or earlier to migrate to 1.0.0,
and any Mill plugins will need to be re-published for Mill-1.0.0/Scala-3

* `build.mill` and `package.mill` files are now written in Scala 3.7 ({link-pr}/3369[#3369]).
...
* `ivyDeps` has been renamed to `mvnDeps`, along with all related tasks and methods
  e.g. `compileIvyDeps` is now `compileMvnDeps` ({link-pr}/4930[#4930]).

* The `Agg()`/`Agg[T]` collection types have been removed and replaced by `Seq()`/`Seq[T]`
  ({link-pr}/4525[#4525]).

* Mill now supports a YAML front-matter syntax in `build.mill` files, defined by line comments
  starting with `//|` ({link-pr}/4969[#4969]). This replaces the `import $ivy...` syntax
  and allows external configuration files like `.mill-version`,
...
** Task implementations can only write to their own `.dest` folder, and only read from their own or upstream `.dest` folders

** Code running during module/task resolution cannot write to disk, and can only perform reads wrapped in `interp.watchValue` blocks

** These filesystem guardrails are best effort to guide users towards best practices, and are not intended to be a fully hermetic sandbox. You can disable them via `os.checker.withValue(os.Checker.Nop){ ... }`
...
** The `.mill` file classpath has been consolidated under
...
** `mill.api`, `mill.define` and `mill.util` have been re-organized. `mill.api` now contains
   `build.mill` code that has zero third-party dependencies, `mill.define` contains the code used
   in `build.mill` that has dependencies, and `mill.util` contains code used in `build.mill`
   that is also used in other parts of Mill. Many classes have been moved around, so if your
   imports no longer compile you may need to try the other two packages to see where the
   class ended up.

* Root modules in `build.mill` and `package.mill` files no longer need an explicit `extends RootModule`
  superclass, as it is now taken care of by Mill automatically ({link-pr}/5008[#5008]) 

* `Jvm.*` methods to spawn subprocesses and classloaders have been consolidated from 20+ helpers
  into 4 main functions ({link-pr}/4456[#4456])

* `evaluator: Evaluator` Command parameters are now only available in commands marked
  `exclusive = true` ({link-pr}/3717[#3717])

* `Task.Source`, `Task.Sources`, and `Task.Input` are no longer allowed to have upstream
  tasks ({link-pr}/4524[#4524]). You can generally replace cases like
  `Task.Sources{ super.sources() ++ Seq(PathRef(...)) }` with
  `def customSources =  Task.Sources()` and `def sources = Task { super.sources() ++ customSources() }`

* `Task` bodies are no longer allowed to write files to disk outside of their `Task.dest`
  folder ({link-pr}/4635[#4635]), except for ``Task.Command``s which are expected to be
  side-effecting. You can disable this check with `os.checker.withValue(os.Checker.Nop) {...}`
  if you have some special reason you need to write a file somewhere and you are sure it is
  safe to do so.
...
* Mill's bootstrap scripts now use the native launcher by the default, except for
  ...
  your `.mill-version` file e.g. `1.0.0-jvm`
...
* `RootModule` and `Cross` can now be applied to the same module ({link-pr}/4593[#4593])
