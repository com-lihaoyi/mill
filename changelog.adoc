...
technical foundation for the next few years of development. We expect it will take some work
from projects on Mill 0.12.x or earlier to migrate to 1.0.0-RC1, and there may be issues in
the release candidate that we hope to surface and fix before 1.0.0 final is released.
Any Mill plugins will need to be re-published for Mill 1.0.0-RC1 if you would like to try them
out in your builds, and re-published again for Mill 1.0.0 final (which may have further changes
and be incompatible with the RC1)
...
* The YAML front-matter allows you to configure arbitrary tasks in the meta-build.
  ...
  meta-build, but other meta-build tasks can be configured as well: `scalaVersion`,
  `scalacOptions`, etc.
...
** `mill.resolve`, `mill.runner`, `mill.eval`, etc. are no longer on the classpath, along
   with many third-party libraries. In general, a lot of Mill packages that were previously
   ...
   `mill.foolib` packages. You can re-include any missing third party libraries you need
...
* `def testParallelism = true` by default, letting Mill run multiple test classes
