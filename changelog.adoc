...
** Code running during module/task resolution cannot write to disk, and can only perform
   reads wrapped in `interp.watchValue` blocks

** These filesystem guardrails are best effort to guide users towards best practices, and are not 
   intended to be a fully hermetic sandbox. You can disable them via `os.checker.withValue(os.Checker.Nop){ ... }`
...
* Mill's server-client model has been simplified, from having an elastic pool of JVM daemons
  ...
  multiple client simultaneously ({link-pr}/5066[#5066]). 
...
** This fixes some long-standing bugs, e.g. `mill shutdown` used to only shutdown the first
   process in the Mill daemon pool and provided no way to shutdown the others, whereas now
