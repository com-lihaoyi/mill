...
* `mill.api.Ctx` is now `mill.api.TaskCtx`, and `mill.api.Ctx` is now `mill.api.ModuleCtx`,
...
* `interp.watch`, `interp.watchValue` and `mill.api.Workspace.workspaceRoot` have been moved
  to a new `mill.api.BuildCtx` object that centralizes all these build-level APIs
  ({link-pr}/5142[#5142])

* Many Task-related renamings in `mill.api`: `Target` -> `Task.Simple`, `TargetImpl` -> `Task.Computed`,
...
** These restrictions are best effort guardrails to guide users towards best practices, and
   ...
   `mill.api.BuildCtx.withFilesystemCheckerDisabled { ... }` if you need to do something
...
** `mill.resolve`, `mill.runner`, `mill.eval`, etc. are no longer on the classpath.
   ...
   need remain exposed under `mill.api`, `mill.api`, `mill.util`, and the various
...
** `mill.api`, `mill.api` and `mill.util` have been re-organized. Many classes have been
...
*** `mill.api` contains the code used in `build.mill` that has dependencies
...
* `mill.api.Cross`: Introduced default cross segments and some CLI convenience
...
* Make `mill.api.Module` a `trait` to allow abstract/virtual modules to be ``trait``s rather than ``class``es
{link-pr}/2536[#2536]
...
* Deprecated `mill.api.SelectMode.Single`
