...
technical foundation for the next few years of backwards-compatible development. We
...
* Mill now supports a YAML front-matter syntax in `build.mill` files, defined by line comments
  starting with `//|` ({link-pr}/4969[#4969]). This replaces the `import $ivy...` syntax with
...
* `build.mill` files are now compiled with a minimized classpath containing only
  ...
  ({link-pr}/4879[#4879]). The `.mill` file classpath has been consolidated under
  the `mill-libs` artifact ({link-pr}/4967[#4967]), which should be used when
  building plugins, rather than the `mill-dist` we were using before.
