...
* Deprecated more API that got remove in Mill 1.0 ({link-pr}/5914[#5914], {link-pr}/6363[#6363])
...
become the default (with an opt out) in Mill 0.13.0. See this blog post for details:
https://mill-build.org/blog/11-jvm-test-parallelism.html[Strategies for Efficiently Parallelizing JVM Test Suites]
