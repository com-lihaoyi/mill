...
* Break the dependency between repositoriesTask and jvmDeps ({link-pr}/4472[#4472])
...
* `runJvmDeps` is now transitive, as is the newly-introduced `runModuleDeps`,
...
* Introduced automatic `PathRef` validation for cached targets; default-enabled it for `CoursierModule.resolveDeps` and various `resolvedJvmDeps` targets
...
* New `JavaModule.mandatoryJvmDeps` target to provide essential dependencies like scala-library without forcing the user to call `super.jvmDeps`
* `ScalaJSModule.scalaLibraryJvmDeps` no longer contains the scala-js-library, but only the scala-library; if you need that, use `ScalaJSModule.mandatoryJvmDeps` instead.
...
* `JavaModule`: Enhanced `jvmDepsTree` to optionally include compile-time and runtime-time dependencies
...
* Publish `compileJvmDeps` as provided scope ({link-issue}/535[535])
...
* Make `jvmDepsTree` properly handle transitive dependencies ({link-issue}/226[#226])
...
* `ScalaModule#repositories`, `scalacPluginJvmDeps`, `scalacOptions`,
...
* Introduced `ScalaModule#jvmDepsTree` command to show dependencies tree
...
* Include `compileJvmDeps` when generating IntelliJ projects
