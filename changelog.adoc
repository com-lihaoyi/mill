...
* Break the dependency between repositoriesTask and libraryDeps ({link-pr}/4472[#4472])
...
* `runLibraryDeps` is now transitive, as is the newly-introduced `runModuleDeps`,
...
* Introduced automatic `PathRef` validation for cached targets; default-enabled it for `CoursierModule.resolveDeps` and various `resolvedLibraryDeps` targets
...
* New `JavaModule.mandatoryLibraryDeps` target to provide essential dependencies like scala-library without forcing the user to call `super.libraryDeps`
* `ScalaJSModule.scalaLibraryLibraryDeps` no longer contains the scala-js-library, but only the scala-library; if you need that, use `ScalaJSModule.mandatoryLibraryDeps` instead.
...
* `JavaModule`: Enhanced `libraryDepsTree` to optionally include compile-time and runtime-time dependencies
...
* Publish `compileLibraryDeps` as provided scope ({link-issue}/535[535])
...
* Make `libraryDepsTree` properly handle transitive dependencies ({link-issue}/226[#226])
...
* `ScalaModule#repositories`, `scalacPluginLibraryDeps`, `scalacOptions`,
...
* Introduced `ScalaModule#libraryDepsTree` command to show dependencies tree
...
* Include `compileLibraryDeps` when generating IntelliJ projects
