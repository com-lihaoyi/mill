...
* The YAML front-matter allows you to configure arbitrary tasks in the meta-build.
  ...
  `scalacOptions`, etc. using the JSON version of the in-memory data structures.
...
* `interp.*` and `mill.api.Workspace.workspaceRoot` have been moved to a new `mill.define.BuildCtx` object

* `mill.testkit.TestBaseModule` has been renamed `mill.testkit.TestRootModule`

* The terms `client`/`server` throughout the codebase have been renamed to `launcher`/`daemon`

* Removed `PublishModule.publishSelfDependency` in favor of `artifactMetadata`
...
** During the Resolution phase, code running during module/task instantiation cannot
   write to disk, and can only perform reads wrapped in `interp.watchValue` blocks
...
** These restrictions are best effort guardrails to guide users towards best practices, and
   ...
   `os.checker.withValue(os.Checker.Nop){ ... }` if you need to do something special and
...
** Similarly, many third-party libraries that were on the Mill classpath by happenstance
   have been removed. You can re-include any missing third party libraries you need
...
* Mill's plugin infrastructure has changed with the release of Mill 1.0.0, and any plugins will 
  need to be updated. See the latest documentation for 
  https://mill-build.org/mill/main-branch/extending/writing-plugins.html[Writing Mill Plugins]
  for how plugins are currently defined and published.
...
* `--notify-watch` now defaults to `true`, but can be disabled by explicitly passing `--notify-watch false`
  to the Mill launcher or in the `.mill-opts` file
