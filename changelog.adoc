...
// find-replace-regex for git shortlog --group=format:online: #(\d+) -> {link-pr}/$1[#$1]
...
=== 2.0.0-M1
:version: 2.0.0-M1
:milestone-name: 2.0.0-M1
:milestone: 146
:prev-version: 1.1.6
_Changes since {prev-version}:_

`1.2.0` is planned to be a binary-compatible but breaking update to the Mill build tool: while
Mill's APIs and method signatures won't change, some third-party plugins or custom build logic
may need to be tweaked for compatibility with the new path-serialization strategy necessary
to support reproducible builds and remote caching.

The headline features of Mill `1.2.0` are *Remote Caching* and a *reproducible `out/` folder*.
Mill now serializes absolute paths under the workspace and home directories as stable relative
aliases, making `out/` independent of where the build lives on disk. This makes task caches
portable across checkouts and machines, which in turn enables Remote Caching: sharing task
outputs (e.g. `compile`, `testCached`) across worktrees, machines, and CI via a cache speaking
the Bazel remote-execution protocol (bazel-remote, Buildbarn, EngFlow) or a shared `file:`/NFS
folder. See https://mill-build.org/mill/main-branch/large/remote-caching.html[Remote Caching] for
details.

* *Remote Caching*: share task outputs across worktrees, machines, and CI via a remote cache,
  enabled with `--remote-cache-location` (or the `mill-remote-cache-location` build-header key).
  Mill supports the Bazel remote-cache protocol and any Bazel-compatible cache servers such as
  e.g. https://github.com/buchgr/bazel-remote[Bazel-Remote].
  See https://mill-build.org/mill/main-branch/large/remote-caching.html[Remote Caching]
  {link-pr}/2777[#2777]

* *Reproducible `out/` folder contents*: absolute paths under the workspace and home directories
  are now serialized as stable relative aliases, so task caches, metadata, and logs no longer
  embed machine-specific absolute paths. See
  https://mill-build.org/mill/main-branch/fundamentals/out-dir.html#_reproducible_out_contents[Reproducible `out/` Contents]
  {link-pr}/4642[#4642]. __**Some user code may need to customize exactly how paths are serialized
  to ensure it works while still maintaining reproducibility**__
  (see https://mill-build.org/mill/main-branch/fundamentals/bundled-libraries.html#_os_lib_path_serialization[Path Serialization])

* *Fine-grained task-level concurrency between launchers*: the Mill daemon now uses per-task
  read/write locking instead of a single global workspace lock, so multiple concurrent `mill`
  commands can run non-conflicting tasks in parallel. See
  https://mill-build.org/mill/main-branch/depth/parallelism.html#_concurrent_mill_invocations[Concurrent Mill Invocations]
  {link-pr}/6955[#6955]

* Split out a `globalExclusive = true` flag from the vanilla `exclusive = true` flag on
  `Task.Command`; `globalExclusive` takes the daemon-wide workspace lock and is used for mutating
  commands like `clean`, `init`, and the formatters. See
  https://mill-build.org/mill/main-branch/fundamentals/tasks.html#_globally_exclusive_commands[Globally-Exclusive Commands]
  {link-pr}/7069[#7069]

* Add a `--replay-logs` option to re-print the logs of cached tasks {link-pr}/7196[#7196]

* `mill init`: Detect Quarkus and Spring Boot projects and apply `QuarkusModule`/`SpringBootModule`.
  See https://mill-build.org/mill/main-branch/migrating/auto-migrating.html[Automated Migration Tools]
  {link-pr}/7117[#7117] {link-pr}/7093[#7093]

* `SpringBootModule` now inherits `RepackageModule` {link-pr}/7120[#7120]

* Dependency strings now support a `force` attribute, e.g. `mvn"org:name:ver;force"`, usable from
  YAML build headers. See
  https://mill-build.org/mill/main-branch/fundamentals/library-deps.html#_forcing_versions[Forcing versions]
  {link-pr}/7108[#7108]

* Add `testBatchFrameworkTasks` to run each fork group as a single batch inside the forked JVM,
  needed by frameworks like Weaver. See
  https://mill-build.org/mill/main-branch/javalib/testing.html#_batch_mode_for_test_frameworks[Batch Mode for Test Frameworks]
  {link-pr}/7119[#7119]

* Add `BuildCtx.rootModule` to give plugins and `Task.Command`s access to the build's root module
  {link-pr}/7090[#7090]

* Add option `-L` as a short alternative to `--meta-level` {link-pr}/7115[#7115]

* Use `-Ymagic-offset-header` to fix up line numbers in build files and scripts, so compiler
  errors and stack traces point at the right lines {link-pr}/5932[#5932] {link-pr}/7262[#7262]

* Update scalafmt to 3.11.0 {link-pr}/7048[#7048]

* Refine defaults for `KotlinModule.kotlinFriendModules` {link-pr}/7191[#7191]

* Android: Only have `AndroidLibModule` for libraries {link-pr}/7034[#7034]

* Make sure Groovy integration works fine with Groovy 6 {link-pr}/7124[#7124]

* Use Java 25 by default {link-pr}/7076[#7076]

* Update Scala.js to 1.21.0 {link-pr}/7049[#7049] and Scala Native to 0.5.12 {link-pr}/7143[#7143]

* Re-add support for dot-config files {link-pr}/7180[#7180]

* Always clean the classes dir when using the Kotlin CLI compiler {link-pr}/7189[#7189]

* Use an absolute path when loading a native library {link-pr}/7208[#7208]

* Fix `mill visualize` with no arguments failing with a `MatchError` {link-pr}/6973[#6973]

* Fix Spotless `RelPathRef` decoding {link-pr}/7111[#7111]

* Fix `*.iml` generation for Kotlin in GenIDEA {link-pr}/7082[#7082]

* Fix fine-grained-locking BSP import deadlock {link-pr}/7161[#7161] and cross-request deadlocks
  from retained per-task read locks {link-pr}/7136[#7136]


[#1.3.0-M1]
=== 1.3.0-M1
:version: 1.3.0-M1
:milestone-name: 1.3.0-M1
:milestone: 152
:prev-version: 1.1.10
_2026-09-24_

This branch is a snapshot of the current development in the `main` branch except the behavioral breaking changes (remote caching and reproducible out).

__Changes since Mill {prev-version}:__
...
* Update Scala to 3.9.0 (backport #7470)
...
* Update Scala.js to 1.22.0 {link-pr}/7049[#7049] and Scala Native to 0.5.12 {link-pr}/7143[#7143]
...
* Make annotation processors not inherit zinc classpath ({link-pr}/7089[#7089])
* GenIDEA: Order IML components lexicographically ({link-pr}/7084[#7084])
* Use `globalExclusive` for Contrib `OwaspDependencyCheckModule` ({link-pr}/7102[#7102])
* Add `mill.eclipse/` entry point and fix active command message ({link-pr}/7139[#7139])
* Fix O(n^2) per-task scan in LeaseTracker.reacquireDropped ({link-pr}/7169[#7169])
* Bound execution-pool threads to `--jobs` on the uncontended fast path ({link-pr}/7170[#7170])
* Simplify LeaseTracker graph traversals; small evaluation/resolution fixes ({link-pr}/7172[#7172])
* Fix concurrency/caching/locking issues from multi-agent review (backport {link-pr}/7176[#7176])
* Clarity/simplification cleanups from multi-agent review (backport {link-pr}/7177[#7177])
* More cleanups and simplifications to executor/evaluator/remote-cache code paths (backport {link-pr}/7178[#7178])
* Re-add support for dot-config files (backport {link-pr}/7180[#7180])
* Various dependency and version updates
* Always clean the classes dir when using the Kotlin CLI compiler (backport {link-pr}/7189[#7189])
* Make scripts use `-Ymagic-offset-header` flags too  ({link-pr}/7262[#7262])
* `mill init`: Pick up Annotation Processors (backport {link-pr}/7200[#7200])
* Revert mill init bloated dependencies and fix SpringBootModule reflection (-parameters) ({link-pr}/7265[#7265])
* Documentation additions for parallelism, deps, testing, migrating (backported)
* Ensure, AssembleTests works with installed Java version 17 (backport {link-pr}/7281[#7281])
* RepackageModule: Move implicit helper module definition into companion object (backport {link-pr}/7280[#7280])
* Fix watch mode (backport {link-pr}/7307[#7307])
* contrib/codeartifact: split dotted group into path segments (backport {link-pr}/7289[#7289])
* Update nativeImageOptions to enable HTTP and HTTPS (backport {link-pr}/7310[#7310])
* Initial implementation for quarkus codegen flow (backport {link-pr}/7296[#7296])
* fix(javalib): resolve relative Zinc product ids (backport {link-pr}/7286[#7286])
* Add ES2022, ES2023, ES2024 to ESVersion enum and JSPI support (backport {link-pr}/7303[#7303])
* `mill init` Spring detection fixes (backport {link-pr}/7349[#7349])
* Expose top-level precompiled YAML modules under BuildCtx.rootModule (backport {link-pr}/7188[#7188])
* `mill init`: Gradle Kotlin Support & Refactor  (backport {link-pr}/7386[#7386])
* `mill init`: Micronaut projects (backport {link-pr}/7418[#7418])
* Introduce `mandatoryBomMvnDeps` for module overrides (backport {link-pr}/7432[#7432])
* `mill init`: Initial Android support (backport {link-pr}/7446[#7446])
* Document build module access from meta-build sources (backport {link-pr}/7507[#7507])
* Document SemanticDB version configuration (backport {link-pr}/7509[#7509])
* Support .sc files as Scala scripts (backport {link-pr}/7512[#7512])
* Restore provenance headers to bootstrap scripts (backport {link-pr}/7510[#7510])
* Batch prompt repaints into one terminal write (backport {link-pr}/7513[#7513])
* Support PGP signing with exported secret subkeys (backport {link-pr}/7514[#7514])
* Discover non-public overrides for task path mapping (backport {link-pr}/7524[#7524])
* Use classpaths for non-JDT Eclipse dependencies (backport {link-pr}/7523[#7523])
* Fix standalone task graphs and Graphviz error hangs (backport {link-pr}/7522[#7522])
* Show direct submodules in inspect output (backport {link-pr}/7508[#7508])
* Allow Java modules to override BSP dependency sources (backport {link-pr}/7517[#7517])
* Restore scalalib PublishModule external resolution (backport {link-pr}/7518[#7518])
* Preserve Quarkus executable dependency metadata (backport {link-pr}/7519[#7519])
* Honor Maven dependency-management optionality semantics (backport {link-pr}/7520[#7520])
* Notify BSP clients after build-definition changes (backport {link-pr}/7521[#7521])
* Introduce KyoTest as TestModule (backport {link-pr}/7484[#7484])
* Fix/quarkus exe artifact resolution and dependencies with missing os placeholders (backport {link-pr}/7552[#7552])
* Android: Aggregate all dirs in android resources, fix for not being processed by the data binding worker under a single dir (backport {link-pr}/7563[#7563])
* Use single bundle when publishing releases (backport {link-pr}/7575[#7575])
* Add a countdown to the CLI progress reporter (backport {link-pr}/7598[#7598]) ({link-pr}/7602[#7602])


[#1-1-10]
=== 1.1.10
:version: 1.1.10
:milestone-name: 1.1.10
:milestone: 151
:prev-version: 1.1.9
_2026-09-21_

* Deduplicate the non-inverted dependency tree to fix `showMvnDepsTree` hanging ({link-pr}/7548[#7548])
* Contrib: SBOM Add purl (backport {link-pr}/7469[#7469]) ({link-pr}/7550[#7550])
* Suggest cleaning-up out/mill-build for parent class issues (backport {link-pr}/6960[#6960]) ({link-pr}/7554[#7554])
* fix: ignore shebang before YAML frontmatter (backport {link-pr}/7565[#7565]) ({link-pr}/7568[#7568])
* fix(javalib): discover package-access main methods on Java 25+ (backport {link-pr}/7557[#7557]) ({link-pr}/7564[#7564])
* Omit checksums for Sonatype signatures (backport {link-pr}/7570[#7570]) ({link-pr}/7576[#7576])
* Publish snapshots with a shared timestamp (backport {link-pr}/7571[#7571]) ({link-pr}/7580[#7580])
* Backport `OsDetectorModule` (backport {link-pr}/7552[#7552] partially) ({link-pr}/7581[#7581])
* Android: Fix missing tools and android hilt module dep resolution (backport {link-pr}/7577[#7577]) ({link-pr}/7579[#7579])
* GenIdea: Fix script module XML due to missing string substitution (backport {link-pr}/7153[#7153]) ({link-pr}/7585[#7585])


[#1-1-9]
=== 1.1.9
:version: 1.1.9
:milestone-name: 1.1.9
:milestone: 150
:prev-version: 1.1.8
_2026-09-07_

* Build with Mill to 1.1.8 and set API baseline ({link-pr}/7420[#7420])
* fix: Avoid watching directory probes (backport {link-pr}/7173[#7173]) ({link-pr}/7433[#7433])
* BuildInfo: Use root-imports to prevent import of colliding sub-packages (backport {link-pr}/7445[#7445]) ({link-pr}/7447[#7447])
* Fix `clean __` deleting Mill's own `out/mill-*` state (backport {link-pr}/7459[#7459]) ({link-pr}/7467[#7467])
* Update jarjar-abrams-core to 1.18.1 ({link-pr}/7534[#7534])
* Normalize mtime to millis for PathRef quick signature rather than use FileTime ({link-pr}/7506[#7506])

[#1-1-8]
=== 1.1.8
:version: 1.1.8
:milestone-name: 1.1.8
:milestone: 149
:prev-version: 1.1.7
_2026-08-07_

* Improved performance for module dependencies {link-pr}/7304[#7304]
* Fixes for IDEA project generator #7313, {link-pr}/7314[#7314]
* Refined defaults for `KotlinModule.kotlinFriendModules` {link-pr}/7315[#7315]
* Fix Kotlin incremental compilation {link-pr}/7320[#7320]
* Make Kotlin classpath snapshotting persistent {link-pr}/7321[#7321]
* Fix bash completion interruptable behavior and silence output {link-pr}/7346[#7346]
* Support Kotlin 2.4 incremental compilation {link-pr}/7355[#7355]
* Fix semanticdb support for Java module with spaces in the source path {link-pr}/7414[#7414]
...
_All changes were backported from the 1.2.0 development branch._

* Fix a stale classes issue in KotlinModule using the CLI compiler {link-pr}/7189[#7189]
* Introduce a `scalafmt-worker` module
* Make annotation processors not inherit zinc classpath {link-pr}/7089[#7089]
* Add option `-L` as short alternative to `--meta-level` {link-pr}/7115[#7115]
* Add `--replay-logs` option to show logs of cached tasks {link-pr}/7263[#7263]
* Fix race-condition in RepackageModule {link-pr}/7300[#7300]
* Deduplicate reverse dependency tree to fix OOM errors in coursier {link-pr}/7301[#7301]
...
* Experimental support for Pre-compiled Declarative modules, which let users define `.mill.yaml`
  files that do not require codegen, speeding up project initialization {link-pr}/6978[#6978]
* Gradle incremental annotation processing metadata support, to make Java annotation processors
  update as necessary during incremental compilation {link-pr}/6999[#6999]
* Escape colon when naming BSP modules to avoid issues on Windows {link-pr}/7001[#7001]
* Support for building Android projects with Scala {link-pr}/6958[#6958]
* Cache loading of `module-deps-config.json` {link-pr}/6970[#6970]
* Fix sandbox initialization in test worker threads {link-pr}/6971[#6971]
* Optimize PromptLogger#logPrefixedLine for high-volume line output {link-pr}/6966[#6966]
* Remove `MillMiscInfo.scala` files from generated `package.mill` files {link-pr}/6983[#6983]
* Move `BuildFileCls` and `RootModule.Info` out of codegen to instead be instantiated reflectively {link-pr}/6985[#6985]
* Create `android-sources.jar` to improve IDE highlighting in Android projects {link-pr}/6957[#6957]
...
* Support for Java 26 {link-pr}/6951[#6951]
* Fix KtLint scanning of source files {link-pr}/6917[#6917]


[#1-1-3]
=== 1.1.3
:version: 1.1.3
:milestone-name: 1.1.3
:milestone: 142
:prev-version: 1.1.2
_2026-03-08_
...
* Print launcher errors rather than hiding them in the logs {link-pr}/6914[#6914]
* Improve precision of codesig callgraph analysis {link-pr}/6880[#6880]
* Allow users to discover tests classes via zinc {link-pr}/6901[#6901]
* Properly propagate incomplete UTF-8 bytes separately to avoid corruption
  during string construction {link-pr}/6895[#6895]
* Switch Kotlin BT API JVM compile path to KotlinToolchains. {link-pr}/6857[#6857]
* Bump Scala to 3.8.2 {link-pr}/6885[#6885]
* Support case-insensitive behavior for `TaskCtx.env` under Windows {link-pr}/6867[#6867]
* Fix slowness in `showMvnDepsTree` {link-pr}/6863[#6863]
* Make `run` interactive by default {link-pr}/6832[#6832]
* Document `-J` prefix in `javacOptions` to customize JVM daemon runtime flags {link-pr}/6803[#6803]


[#1-1-2]
=== 1.1.2
:version: 1.1.2
:milestone-name: 1.1.2
:milestone: 141
:prev-version: 1.1.1
_2026-02-07_
...
* Fix escaping of strings in yaml lists in `./mill init` {link-pr}/6798[#6798]

* Fix incremental compilation problems with `package.mill.yaml` files {link-pr}/6792[#6792]

* Fix leakage of `org.scalafmt.interfaces.Scalafmt` internal classloaders {link-pr}/6785[#6785]

* Report an error when `unmanagedClasspath` entries are not found on disk {link-pr}/6781[#6781]

* Silence noisy logback/coursier error {link-pr}/6778[#6778]

* Add `-H:-ParseRuntimeOptions` to native image options to fix issue propagating `-D<key>=<foo>`
  arguments{link-pr}/6774[#6774]
...
* Updated `build.mill` files to use to Scala 3.8.2 final {link-pr}/6654[#6654]
