...
override def tokenLookup: GitlabTokenLookup = new GitlabTokenLookup {
  ...
  override def personalTokenFileWD = wd => wd/".mill"/"gitlab"/"token.file"
}
----

This still keeps the default search order, but allows changes to places where to look from. Note how workspace definition is bit different. Instead of value it is a function `os.Path => os.Path` where parameter is the location of workspace.


==== Add or change tokenSearchOrder

If original search order is too wide, or you would like to add places to look you can override the `tokenSearchOrder`. Example below ignores default search order and adds five places to search from.
...
override def tokenLookup: GitlabTokenLookup = new GitlabTokenLookup {
  ...
  override def tokenSearchOrder: Seq[GitlabToken] = Seq(
    ...
    Deploy(WorkspaceFile(wd => wd/"deploy-token")),
...
4. `WorkspaceFile`: From file in workspace.
