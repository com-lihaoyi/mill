...
But depending on the Scala version you are using, there is no matching Ammonite release available.
...
[TIP]
--
_Why is the Ammonite tied to the exact Scala version?_

This is because Ammonite depends on the Scala compiler.
In contract to the Scala library, compiler releases do not guarantee any binary compatibility between releases.
