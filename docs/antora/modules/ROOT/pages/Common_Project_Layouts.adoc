...
If you are following the [Scala.js Tutorial](http://www.scala-js.org/doc/tutorial/basic/), in the section where you test the code, it's mentioned that you need to make the DOM available to Node.js. Install the `jsdom` node library as instructed then add `def jsEnvConfig = T(JsEnvConfig.JsDom())` to your `build.sc` object. In Scala 3, you don't need the "scalajs-env-jsdom-nodejs" mentioned in the tutorial.
