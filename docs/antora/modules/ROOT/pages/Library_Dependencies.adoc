...
Mill uses [coursier] to resolve and download dependencies.
Once downloaded, they are located in the coursier specific cache locations.
For more details about coursier, refer to the [coursier documentation].
...
I Mill we use the additional term `artifactId` which is identical to the `name` when used in the normal form shown above.
When a different for is used, e.g. some double-colons are used between the parts, the `artifactId` typically contains suffixes, but the name doesn't.
...
That means, mixing dependencies of different binary platform will result in non-working runtime and obsure and hard to debug issues.
...
Since the release of Scala 3, the binary compatibility story for Scala changed.
...
If your Scala version is a Scala 3, but you want to use the Scala 2.13 version of a specific library, you can use the `.withDottyCompat` method on that dependency.
...
For convenience, most modules already come with a pre-configured traits for a test submodule,
which already inherits all dependencies of their parent module.
If you need additional test dependencies, you simply add them by overriding `def ivyDeps`, as you would do with normal dependencies.
...
When Mill generated `pom.xml` files to interact with Maven repositories, such compile-only dependencies are mapped to the `provided` scope.

Please note, that dependencies with `provided` scope will never ber resolved transitively. Hence, the name "provided", as the target runtime needs to "provide" them, if they are needed.


== Runtime dependencies

If you want to declare dependencies to be used at runtime, you can use the `runIvyDeps` targets.
...
It is also possible to use higher version of the same library dependencies already  defined in `ivyDeps`, to ensure you can compile against a minimal API version, but run with the latest available version.
