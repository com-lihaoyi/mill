...
Webpack support for Scala JS projects allows including npm dependencies and extending the webpack configuration. It generates source maps for development bundles.

It also bundles Javascript sources from Ivy dependencies that were created by the SBT `scalajs-bundler` plugin. 
...
class ExampleProjectModule(jsCompilationMode: String) extends ScalaJSWebpackModule {
  ...
  override def npmDevDeps = Agg(
    "aNpmDevelopmentDependency" -> "1.0.1"
  )
...
The webpack configuration can be extended with custom configuration files. Each file may only contain one configuration object, which is merged with the generated configuration.

Potentially required plugins have to be added to the `npmDevDeps` target.

Example of a custom config file `custom-webpack.config` placed inside the module directory:
```javascript
...
The different compilation modes are accessible with the `dev`/`prod` cross parameters and can be used to run the different Scala JS compilation modes.
...
The following targets can be overridden to configure the webpack module:

- `webpackOutputPath` specifies the folder in which all webpack artifacts are placed

- `webpackBundleFilename` specifies the name of the bundle generated by webpack
...
- `webpackMergeVersion` => `webpack-merge` version

- `webpackCliVersion` => `webpack-cli` version

- `sourceMapLoaderVersion` => `source-map-loader` version

- `scalaJsFriendlySourceMapLoaderVersion` => `scalajs-friendly-source-map-loader` version
