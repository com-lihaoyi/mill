...
to several days or more for larger projects. These steps come experience doing proof-of-concept
migrations a range of real-world projects to Mill, from simple single-module codebases to
...
** This period gives to time to be confident in the robustness of the new Mill build system,
...
* Applications may need to use frameworks like xref:javalib/web-examples.adoc#_spring_boot_hello_world_app[Spring Boot]
  or xref:javalib/web-examples.adoc#_micronaut_hello_world_app_[Micronaut]
...
However, while _tedious_, such migrations are not usually not _difficult_. Most
build systems use a relatively small set of third-party tools with small amounts of
custom logic, and Mill has built-in integrations with all the common JVM tools and
