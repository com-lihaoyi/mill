...
* **Mill makes customizing the build tool much easier than Maven**. Projects usually grows
...
* **Mill automatically caches and parallelizes your build**: This applies not just to the
  built-in tasks that Mill ships with, but also any custom logic the
  developer might write. This maximizes performance and snappiness of
...
* **Mill follows Gradle's conciseness**: Rather than pages and pages of verbose XML, every
  ...
  While verbosity isn't everything, skimming and understanding a 100-line Mill `build.sc` file is
...
* **Mill builds are more easily understandable**: While the impact of a Gradle config change can
  ...
  only affecting downstream tasks. Apart from humans, tooling
  also finds it easier to understand Mill builds: your IDE can jump to the definition of
  a Mill task, find its usage, peek at its doc-comments. Your IDE can navigate Mill's
...
* **Mill enforces best practices by default**. All Mill tasks are cached by default, even
  custom tasks. All Mill tasks write their output to disk xref:Out_Dir.adoc[following a
  standard convention]. All task inter-dependencies are automatically captured, without
  needing explicit annotation. All Mill builds are incremental, not just tasks but also
  ...
  to maintain the performance and structure of your build, Mill automates that effort so the
