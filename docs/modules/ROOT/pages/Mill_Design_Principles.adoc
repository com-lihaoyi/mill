...
``Target``s available: just add a new `def foo = Task {...}` somewhere and you'll be
able to run it.
...
Every `Target` in a build, defined by `def foo = Task {...}`, is cached by default.
...
with other targets that have their own `Task.dest` folders. In effect, this makes
