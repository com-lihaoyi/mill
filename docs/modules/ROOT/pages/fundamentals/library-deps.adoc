...
* You declare a dependency `val deps = Agg(mvn"com.lihaoyi::fansi:0.2.14")`
* There is another dependency, `val deps = Agg(mvn"com.lihaoyi::PPrint:0.8.1")`
...
* If you want to force to the older version (to prevent it being evicted, and replaced by 0.4.0) then you can use `val deps = Agg(mvn"com.lihaoyi::fansi:0.2.14".forceVersion())`
