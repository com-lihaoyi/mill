...
xref:why-mill.adoc[2-10x speedups] over existing Java build tools
...
an order of magnitude easier than traditional tools, democratizing project builds to anyone
even if they are not build tool experts.
...
from JVM languages like Java/Scala/Kotlin to xref:extending/example-typescript-support.adoc[Typescript]
and xref:extending/example-python-support.adoc[Python]. These features are not as highly-scalable as their Bazel equivalents, but
