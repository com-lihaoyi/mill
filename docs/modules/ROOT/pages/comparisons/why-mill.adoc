...
For this article, we will focus on two benchmarks. First, *Parallel Clean Compile All*

This benchmark involves running `clean` to delete all generated files and re-compiling
everything in parallel.

* Maven parallelism is opt-in via `-T 10`, while for Mill it is enabled by default.
* Maven tests and linters are opt-out via the `-D` properties, while in Mill
