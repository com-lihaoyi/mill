...
On modern hardware and software, Java source code should compile at >100,000 lines per
second, i.e. a million-line Java codebase should clean compile in <10 seconds on a single
thread. Anything less than that is due to overhead from your build system and other tooling.
...
appear to go down in larger builds. Although this `Clean Compile Single-Module` only
deals with compiling a single small module, the `Sequential Clean Compile` benchmarks which
compile entire projects show similar build-tool slowdowns:
...
with Mill reaching ~25,000 lines/s, all of which are far below the 100,000 lines/s
that we should expect from Java compilation.
...
in progress, and will reain so until compiling a 30-40kLOC Java codebase from the command line
takes ~0.3 seconds.