...
{mill-github-url}[Mill] is a Java build tool that aims for performance,
...
It tries to learn from the strengths of each tool, while improving on their weaknesses. Mill
build files are written in Scala, but you do not need to have prior experience in Scala to
read or write them.

Compared to SBT:

* **Mill makes customizing the build yourself much easier than SBT**: is all build tools
  do is move files around and call subprocesses, and Mill makes doing that yourself easy.
...
* **Mill is much more performant than SBT**: SBT has enough overhead that even a dozen
...
* **Mill is much easier to understand than SBT**: Your Mill build is made of bog-standard
  ...
  Mill's build files are much more
