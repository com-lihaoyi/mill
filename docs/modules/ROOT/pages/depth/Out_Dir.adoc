...
rebuilt, e.g. by deleting the `+out/main/+` or `+out/main/compile.*+` folders, but we strongly encourage you to use the xref:scalalib/Scala_Builtin_Commands.adoc#_clean[`clean` command] instead.
...
include::partial$example/depth/out-dir/1-custom-out.adoc[]
