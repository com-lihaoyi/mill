...
trait TutModule extends ScalaModule {
  ...
  /**
    ...
    */
  def tutClasspath: T[Agg[PathRef]] =
    T {
  ...
  def tut: T[os.CommandResult] =
    T {
      ...
      os.proc(
          'java,
          "-cp",
          tutClasspath().map(_.path.toIO.getAbsolutePath).mkString(java.io.File.pathSeparator),
          "tut.TutMain",
          tutArgs
        )
        .call(millSourcePath)
