  ...
  def routerClasspath: T[Seq[PathRef]] = Task {
    defaultResolver().classpath(
      playMinorVersion() match {
        case "2.6" | "2.7" | "2.8" =>
          Seq(jvm"com.typesafe.play::routes-compiler:${playVersion()}")
        case "2.9" =>
          Seq(jvm"com.typesafe.play::play-routes-compiler:${playVersion()}")
        case _ =>
          Seq(jvm"org.playframework::play-routes-compiler:${playVersion()}")
      }
    )
  }
