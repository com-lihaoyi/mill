...
object PlayModuleTests extends TestSuite with PlayTestSuite {
  ...
  object playmulti extends TestUtil.BaseModule{
    ...
    class CoreCrossModule(val crossScalaVersion: String, crossPlayVersion: String) extends PlayModule {
  ...
  def tests: Tests = Tests {
      ...
      test("fromBuild") {
        matrix.foreach { case (scalaVersion, playVersion) =>
          workspaceTest(playmulti) { eval =>
            ...
            val Right((resources, _)) = eval.apply(playmulti.core(scalaVersion, playVersion).resources)
            val Right((testSources, _)) = eval.apply(playmulti.core(scalaVersion, playVersion).test.sources)
            val Right((testResources, _)) = eval.apply(playmulti.core(scalaVersion, playVersion).test.resources)
            assert(
    ...
    test("dependencies") {
      test("fromBuild") {
        matrix.foreach { case (scalaVersion, playVersion) =>
          workspaceTest(playmulti) { eval =>
            val Right((deps, evalCount)) = eval.apply(playmulti.core(scalaVersion, playVersion).ivyDeps)
    ...
    test("compile") {
      matrix.foreach { case (scalaVersion, playVersion) =>
        workspaceTest(playmulti) { eval =>
          ...
          val Right((_, unchangedEvalCount)) = eval.apply(playmulti.core(scalaVersion, playVersion).compile)
