...
object PlayModuleTests extends TestSuite with PlayTestSuite {
  ...
  object playmulti extends TestBaseModule {
    ...
    trait CoreCrossModule extends PlayModule with Cross.Module2[String, String] {
      ...
      override def libraryDeps = Task { super.libraryDeps() ++ Seq(ws()) }
    }
    ...
    test("dependencies") {
      test("fromBuild") {
        matrix.foreach { case (scalaVersion, playVersion) =>
          UnitTester(playmulti, resourcePath).scoped { eval =>
            val Right(result) =
              eval.apply(playmulti.core(scalaVersion, playVersion).libraryDeps): @unchecked
      ...
      test("resolvedRunLibraryDeps") {
        matrix.foreach { case (scalaVersion, playVersion) =>
          UnitTester(playmulti, resourcePath).scoped { eval =>
            val Right(_) =
              eval.apply(playmulti.core(scalaVersion, playVersion).resolvedRunLibraryDeps): @unchecked
          }
        }
      }
    }
