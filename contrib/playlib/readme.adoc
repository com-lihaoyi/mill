...
If you want to add an optional library using the helper you can do so by overriding `libraryDeps`
...
object core extends PlayApiModule {
    ...
    override def libraryDeps = Task { super.libraryDeps() ++ Seq(ws(), filters()) }
}
----
