...
case class BspConfigJson(name: String, argv: Seq[String], version: String, bspVersion: String, languages: Seq[String])
...
object BSP extends ExternalModule {
  ...
  def install(ev: Evaluator): Command[Unit] =
    T.command {
      ...
      try os.write(bspDirectory / "mill.json", createBspConnectionJson())
      catch {
  ...
  def createBspConnectionJson(): String = {
    val millPath =
      Option(mill.modules.Util.millProperty("MILL_CLASSPATH")).getOrElse(System.getProperty("MILL_CLASSPATH"))
