...
trait BuildInfo extends ScalaModule {
  ...
  def generatedBuildInfo: T[(Seq[PathRef], PathRef)] =
    T {
      ...
      if (members.nonEmpty) {
        ...
        val internalMembers =
          members
            ...
            .mkString("\n")
        logger.debug(
          s"Generating object [${buildInfoPackageName.map(_ + ".").getOrElse("")}$buildInfoObjectName] with [${members.size}] members to [$outputFile]"
        )
        os.write(
          outputFile,
