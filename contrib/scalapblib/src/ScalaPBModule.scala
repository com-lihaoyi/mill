...
trait ScalaPBModule extends ScalaModule {
  ...
  def scalaPBSources: Sources =
    T.sources {
      millSourcePath / 'protobuf
    }

  def scalaPBOptions: T[String] =
    T {
      (
        (if (scalaPBFlatPackage()) Seq("flat_package") else Seq.empty) ++
          ...
          else if (Version(scalaPBVersion()) >= Version("0.7.0"))
  ...
  def scalaPBClasspath: T[Loose.Agg[PathRef]] =
    T {
      resolveDependencies(
        Seq(
  ...
  def scalaPBProtoClasspath: T[Agg[PathRef]] =
    T {
