...
trait ScalaJSWebpackModule extends ScalaJSModule {
  ...
  // Custom webpack config that gets merged with the generated config
  def customWebpackConfigOpt: Target[Option[os.Path]] = None
  ...
  def writeWpConfig: Task[
    ...
    T.task {
      (dst: ops.Path,
       cfg: String,
       customCfgPathOpt: Option[os.Path],
       io: String,
       out: String,
       opt: Boolean) =>
        val cfgString = ujson
        ...
        val generatedCfg =
          s"""// Webpack config generated by ScalaJSWebpackModule
             ...
             |""".stripMargin
        val mergedCfg = customCfgPathOpt match {
          case None =>
            s"""|$generatedCfg
                |module.exports = $generatedCfgName;
                |""".stripMargin
          case Some(path) =>
            val customCfgString = readStringFromInputStream(path.getInputStream)
            ...
            val customCfg =
              s"""// Custom Webpack config from '$path', defined in ScalaJSWebpackModule
                 ...
                 |""".stripMargin
            val webpackMergeImport = "const merge = require('webpack-merge');\n"
  ...
  def webpack: Task[(ops.Path, ops.Path, Boolean) => Unit] = T.task {
    (src: ops.Path, dst: ops.Path, opt: Boolean) =>
      ...
      writeWpConfig()
        .apply(dst, cfg, customWebpackConfigOpt(), outJs.toString, bundleFilename(), opt)
