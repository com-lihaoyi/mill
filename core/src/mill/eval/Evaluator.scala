...
case class Labelled[T](task: NamedTask[T], segments: Segments) {
  def format = task match {
    case t: Target[T] =>
      Some(t.readWrite.asInstanceOf[upickle.default.ReadWriter[T]])
    case _ => None
  }
  def writer = task match {
    case t: mill.define.Command[T] =>
      Some(t.writer.asInstanceOf[upickle.default.Writer[T]])
    case t: Target[T] =>
      Some(t.readWrite.asInstanceOf[upickle.default.ReadWriter[T]])
    case _ => None
  }
...
case class Evaluator[T](home: Path,
  ...
  def evaluate(goals: Agg[Task[_]]): Evaluator.Results = {
    ...
    val timings =
      mutable.ArrayBuffer.empty[(Either[Task[_], Labelled[_]], Int, Boolean)]
    ...
    val failing = new util.MultiBiMap.Mutable[Either[Task[_], Labelled[_]],
                                              Result.Failing[_]]
    for ((k, vs) <- sortedGroups.items()) {
      failing.addAll(
        k,
        vs.items.flatMap(results.get).collect {
          case f: Result.Failing[_] => f.map(_._1)
        }
      )
    }
    write.over(
  ...
  def evaluateGroupCached(terminal: Either[Task[_], Labelled[_]],
    ...
    terminal match {
      ...
      case Right(labelledNamedTask) =>
        ...
        val cached = for {
          cached <- try Some(
            upickle.default.read[Evaluator.Cached](paths.meta.toIO))
          catch { case e: Throwable => None }
          if cached.inputsHash == inputsHash
          reader <- labelledNamedTask.format
          parsed <- try Some(upickle.default.read(cached.value)(reader))
          catch { case e: Throwable => None }
        ...
        workerCached.map((_, inputsHash)) orElse cached match {
          case Some((v, hashCode)) =>
            val newResults =
              mutable.LinkedHashMap.empty[Task[_], Result[(Any, Int)]]
          ...
          case _ =>
            ...
            val msgParts = Seq(first.asInstanceOf[Segment.Label].value) ++ rest
              .map {
  ...
  def destSegments(labelledTask: Labelled[_]): Segments = {
    ...
    if (ctx.foreign) {
      ...
      val relative = labelledTask.task.ctx.millSourcePath
        .relativeTo(rootModule.millSourcePath)
      // Encoding the number of `/..`
      val ups =
        if (relative.ups > 0) Segments.labels(s"up-${relative.ups}")
        else Segments()
      Segments
        .labels(prefix)
        .++(ups)
  ...
  def handleTaskResult(v: Any,
    ...
    labelledNamedTask.task.asWorker match {
      ...
      case None =>
        val terminalResult = labelledNamedTask.writer
                ...
                Console.withErr(multiLogger.errorStream) {
                  try task.evaluate(args)
                    ...
                    case NonFatal(e) =>
                      Result.Exception(
                        e,
                        new OuterStack(new Exception().getStackTrace))
                  }
                }
              }
            }
...
object Evaluator {
  case class Cached(value: Js.Value, valueHash: Int, inputsHash: Int)
  object Cached {
    implicit val rw: upickle.default.ReadWriter[Cached] =
  ...
  object Timing {
    implicit val readWrite: upickle.default.ReadWriter[Timing] =
      upickle.default.macroRW
  }
  case class Results(
    ...
    val sortedGroups = Graph.groupAroundImportantTargets(topoSorted) {
      case t: NamedTask[Any] =>
        ...
        val finalTaskOverrides = t match {
          case t: Target[_] =>
            rootModule.millInternal.segmentsToTargets
              .get(segments)
              .fold(0)(_.ctx.overrides)

          case c: mill.define.Command[_] =>
            def findMatching(
                cls: Class[_]): Option[Seq[(Int, EntryPoint[_])]] = {
        ...
        val additional =
          if (finalTaskOverrides == t.ctx.overrides) Nil
            ...
            Seq(Segment.Label("overriden")) ++ t.ctx.enclosing
              .split("\\.|#| ")
              .map(Segment.Label)
