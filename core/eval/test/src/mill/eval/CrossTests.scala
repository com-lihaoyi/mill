...
object CrossTests extends TestSuite {
  val tests = Tests {
    ...
    test("crossExtension") {
      ...
      val Right(Result("Param Value: a", _)) = check(crossExtension.myCrossExtended("a", 1).param1): @unchecked
      val Right(Result("Param Value: 2", _)) = check(crossExtension.myCrossExtended("b", 2).param2): @unchecked
    ...
    test("doubleCross") {
      ...
      val Right(Result("212_native", 1)) = check(doubleCross.cross("212", "native").suffix): @unchecked
    }
    ...
    test("innerCrossModule") {
      ...
      val Right(Result("foo a", 1)) = check(innerCrossModule.myCross3("a", 1, true).foo.bar): @unchecked
      val Right(Result("foo 1", 1)) = check(innerCrossModule.myCross3("a", 1, true).foo.qux): @unchecked
      val Right(Result("foo true", 1)) = check(innerCrossModule.myCross3("a", 1, true).foo.lol): @unchecked
      val Right(Result("baz b", 1)) = check(innerCrossModule.myCross3("b", 2, false).baz.bar): @unchecked
      val Right(Result("baz 2", 1)) = check(innerCrossModule.myCross3("b", 2, false).baz.qux): @unchecked
      val Right(Result("baz false", 1)) = check(innerCrossModule.myCross3("b", 2, false).baz.lol): @unchecked
    }

    test("nestedCrosses") {
      val check = UnitTester(nestedCrosses, null)

      val Right(Result("210_jvm", 1)) = check(nestedCrosses.cross("210").cross2("jvm").suffix): @unchecked
      val Right(Result("210_js", 1)) = check(nestedCrosses.cross("210").cross2("js").suffix): @unchecked
      val Right(Result("211_jvm", 1)) = check(nestedCrosses.cross("211").cross2("jvm").suffix): @unchecked
      val Right(Result("211_js", 1)) = check(nestedCrosses.cross("211").cross2("js").suffix): @unchecked
      val Right(Result("212_jvm", 1)) = check(nestedCrosses.cross("212").cross2("jvm").suffix): @unchecked
      val Right(Result("212_js", 1)) = check(nestedCrosses.cross("212").cross2("js").suffix): @unchecked
      val Right(Result("212_native", 1)) = check(nestedCrosses.cross("212").cross2("native").suffix): @unchecked
    }
    ...
    test("nestedTaskCrosses") {
      ...
      val Right(Result("210_jvm_1", 1)) = check(model.cross1("210").cross2("jvm").suffixCmd("1")): @unchecked
      val Right(Result("210_js_2", 1)) = check(model.cross1("210").cross2("js").suffixCmd("2")): @unchecked
      val Right(Result("211_jvm_3", 1)) = check(model.cross1("211").cross2("jvm").suffixCmd("3")): @unchecked
      val Right(Result("211_js_4", 1)) = check(model.cross1("211").cross2("js").suffixCmd("4")): @unchecked
      val Right(Result("212_jvm_5", 1)) = check(model.cross1("212").cross2("jvm").suffixCmd("5")): @unchecked
      val Right(Result("212_js_6", 1)) = check(model.cross1("212").cross2("js").suffixCmd("6")): @unchecked
