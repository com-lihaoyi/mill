...
import mill.api.{Evaluator, ExternalModule, Result, PrecompiledModule}
    ...
    if (allErrors.nonEmpty) {
      ...
      scriptCls.flatMap(
        instantiate(
          ...
          scriptText,
          PrecompiledModule.Config(scriptFile, moduleDeps, compileModuleDeps, runModuleDeps, headerData)
        )
      )
    }
  }
