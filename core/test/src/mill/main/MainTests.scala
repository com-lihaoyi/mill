  ...
  val tests = Tests {
    ...
    'single - {
      ...
      'neg1 - check(
        ...
        Discover[singleton.type],
        "doesntExist",
        Left("Cannot resolve task doesntExist")
      )
      'neg2 - check(
        singleton,
        Discover[singleton.type],
        "single.doesntExist",
        Left("Cannot resolve module single")
      )
      'neg3 - check(
        singleton,
        Discover[singleton.type],
    ...
    'nested - {
      'pos1 - check(
        nestedModule,
        Discover[nestedModule.type],
        "single",
        Right(nestedModule.single)
      )
      'pos2 - check(
        nestedModule,
        Discover[nestedModule.type],
        "nested.single",
        Right(nestedModule.nested.single)
      )
      'pos3 - check(
        nestedModule,
        Discover[nestedModule.type],
        "classInstance.single",
        Right(nestedModule.classInstance.single)
      )
      'neg1 - check(
        nestedModule,
        Discover[nestedModule.type],
      ...
      'neg4 - check(
        ...
        Left("Cannot resolve task classInstance.doesntExist")
      )
    }
    'cross - {
      'single - {
        'pos1 - check(
          ...
          "cross[210].suffix",
          Right(singleCross.cross("210").suffix)
        )
        'pos2 - check(
          singleCross,
          Discover[singleCross.type],
          "cross[211].suffix",
          Right(singleCross.cross("211").suffix)
        )
        'neg1 - check(
          singleCross,
          Discover[singleCross.type],
          "cross[210].doesntExist",
          Left("Cannot resolve task cross[210].doesntExist")
        )
        'neg2 - check(
          singleCross,
          Discover[singleCross.type],
          "cross[doesntExist].doesntExist",
          Left("Cannot resolve cross cross[doesntExist]")
        )
        'neg2 - check(
          singleCross,
          Discover[singleCross.type],
