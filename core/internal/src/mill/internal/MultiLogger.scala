...
private[mill] class MultiLogger(
  ...
  def prompt: Logger.Prompt = new Logger.Prompt {
    ...
    override def infoColor(s: String): String = logger1.prompt.infoColor(logger2.prompt.infoColor(s))
    override def warnColor(s: String): String = logger1.prompt.warnColor(logger2.prompt.warnColor(s))
    override def errorColor(s: String): String = logger1.prompt.errorColor(logger2.prompt.errorColor(s))
