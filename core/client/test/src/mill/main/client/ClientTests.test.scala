...
object ClientTests extends TestSuite {
  ...
  def readSamples(samples: String*): Array[Byte] = {
    ...
    for (sample <- samples) {
      ...
      println(sample)
      // println(this.getClass().getResource(sample).toString())

      // println(this.getClass.getResourceAsStream(sample))
      val content = this.getClass.getResourceAsStream(sample).readAllBytes()
      // val content = os.read.bytes(os.Path("/Users/simon/Code/mill_client/client/test/resources" + samples(0)))
