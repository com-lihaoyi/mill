...
These range from tiny unit tests, to larger integration tests that instantiate a `MillTestKit.BaseModule` in-process and a `TestEvaluator` to evaluate tasks on it.
