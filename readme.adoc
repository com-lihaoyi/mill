...
* The `Task.workspace` context path now always points to the top-level project directory, also for meta builds {link-pr}/2809[#2809]
...
* You can now override `Task {...}` ``Target``s with `Task.source` or `Task.sources`, and vice versa {link-pr}/2402[#2402]
...
* `core`: We made slight adaptions to the `Task.dest` location of target defined in included files, to fix potential colliding cache locations
...
* New `Task.ctx().workspace` API to access the project root directory
...
* Alias `Task.ctx.*` functions to `Task.*`: `Task.dest`, `Task.log`, etc.
