...
and all associated `.sc` files have been renamed to `.mill`. All `.mill` files must have a `package` declaration,
with `package build` for `.mill` files in the root folder and `package build.foo.bar` for `.mill` files in `foo/bar/`.

*** For migration compatibility, `.sc` files are still supported, but you should convert to use `.mill` with explicit
`package` declarations to benefit from improved IDE support
...
** Only `foo.mill` files in folders adjacent to a `build.mill` or `package.mill` can be imported via `import $file`.

*** You need to create empty `package.sc` files in any folder containing scripts if they are not already
adjacent to your `build.sc` in the root folder
