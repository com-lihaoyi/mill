...
| Sub-Process w/ packaging/publishing | `example.\\__.assembly.server`, `integration.__.assembly.server` | `dist.assembly` | `test-mill-release.sh`
...
`example` and `integration` tests to save time, and only run `.assembly.fork`
or `.assembly.server` once `.local` is passing.
