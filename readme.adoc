...
The upcoming 0.12.0 release is _binary compatible_ with 0.11.x, but contains a lot of
breaking changes that are necessary to move the project forward. The breaking changes
are listed below, with instructions provided on how to either disable the breaking change
with a flag or adjust your code to accommodate it. Due to binary compatibility, Mill plugins
published for Mill 0.11.x can continue to be used without updating them.

0.12.x is intended to bundle together all the breaking changes we need to do that do not
break binary compatibility. It is expected to be followed by 0.13.x, which will contain
further changes that _do_ break binary compatibility.
