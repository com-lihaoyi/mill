    ...
    trait KotlinVersionCross extends KotlinModule with Cross.Module2[String, Boolean] {
      ...
      object test extends KotlinTests with TestModule.Junit4 {
        override def jvmDeps = super.jvmDeps() ++ Seq(
          jvm"org.jetbrains.kotlin:kotlin-test-junit:${this.kotlinVersion()}"
        )
      }
      object kotest extends KotlinTests with TestModule.Junit5 {
        override def jvmDeps = super.jvmDeps() ++ Seq(
          jvm"io.kotest:kotest-runner-junit5-jvm:${junit5Version}"
        )
      }
    }
  ...
  def tests: Tests = Tests {
    ...
    test("compile") {
      ...
      HelloKotlin.main.crossModules.foreach(m => {
        val Right(compiler) = eval.apply(m.kotlinCompilerJvmDeps): @unchecked
    ...
    test("testCompile") {
      ...
      HelloKotlin.main.crossModules.foreach(m => {
        val Right(compiler) = eval.apply(m.test.kotlinCompilerJvmDeps): @unchecked
