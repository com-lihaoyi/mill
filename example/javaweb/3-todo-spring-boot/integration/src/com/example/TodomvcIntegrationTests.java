...
class TodomvcIntegrationTests {
	...
	void addNewTodoItem() {
		...
		ResponseEntity<String> postResponse = this.restTemplate.exchange("http://localhost:" + port + "/", HttpMethod.POST, entity, String.class);
		assertThat(postResponse.getStatusCode().is3xxRedirection()).isTrue();

		// Send a GET request to verify the new todo item was added
		ResponseEntity<String> getResponse = this.restTemplate.getForEntity("http://localhost:" + port + "/", String.class);
